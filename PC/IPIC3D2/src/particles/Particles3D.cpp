...
void Particles3D::delete_outside_particles(){
  ...
  // Do not need to check whether this is a boundary processor. Because
  // there are fast moving particles which can move across more than one
  // processor and leave the computational domain within one step.  
  const bool fluidXleft = !vct->getPERIODICX_P() &&  bcPfaceXleft == FLUID;
  const bool fluidYleft = !vct->getPERIODICY_P() &&  bcPfaceYleft == FLUID;
  const bool fluidZleft = !vct->getPERIODICZ_P() &&  bcPfaceZleft == FLUID;
						 
  const bool fluidXright = !vct->getPERIODICX_P() && bcPfaceXright == FLUID;
  const bool fluidYright = !vct->getPERIODICY_P() && bcPfaceYright == FLUID;
  const bool fluidZright = !vct->getPERIODICZ_P() && bcPfaceZright == FLUID;
...
#pragma omp for schedule(static)
	...
	for(int c=0; c<8; c++)
	  {
	    Bxl0 += weights[c] * field_components[c][0];
	    Byl0 += weights[c] * field_components[c][1];
	    Bzl0 += weights[c] * field_components[c][2];
	  }
	const double Bt0= max(sqrt(Bxl0*Bxl0+Byl0*Byl0+Bzl0*Bzl0),1e-99);
	// dt_sub <= T/8, where T is the gyroperiod.
	dt_sub = M_PI*c/(4*abs(qom)*Bt0);
	sub_cycles = int(dt/dt_sub)+1;
      }else{
	dt_sub=dt;
	sub_cycles=1;
      }
      ...
      dto2_sub = .5 * dt_sub;
      qdto2mc_sub = qom*dto2_sub/c;
      subcycle_sum += sub_cycles;

      //--------determine # of subcyles END-------------
...
#pragma simd
	  ...
	  const pfloat denom = 1.0 / (1.0 + omsq);
	  // solve the position equation
	  const pfloat ut = uorig + qdto2mc_sub * Exl;
	  const pfloat vt = vorig + qdto2mc_sub * Eyl;
	  const pfloat wt = worig + qdto2mc_sub * Ezl;
	  //const pfloat udotb = ut * Bxl + vt * Byl + wt * Bzl;
	  const pfloat udotOm = ut * Omx + vt * Omy + wt * Omz;
	  // solve the velocity equation
	  uavg = (ut + (vt * Omz - wt * Omy + udotOm * Omx)) * denom;
