...
\hline \\
A    & {\tt 1:MaxBlock*nProc}          &  \red{A}ll blocks on all processors \\
B    & {\tt 1:MaxBlock}                &  \red{B}locks on one processor \\
C    & {\tt 1:nI,1:nJ,1:nK}            &  \red{C}ell centers without ghost cells \\
C    & {\tt 1:MaxComp}                 &  \red{C}omponents of the SWMF \\
D    & {\tt 1:nDim} or {1:MaxDim}      &  \red{D}imensions (of the grid) \\
E    & {\tt 1:2*nDim}                  &  \red{E}dges (of a block) \\
F    & {\tt 1:nI+1,1:nJ+1,1:nK+1}      &  \red{F}aces (of cells) \\
G & {\tt MinI:MaxI,MinJ:MaxJ,MinK:MaxK} & \red{G}host and physical cells \\
I    & {\tt ?:?}                       &  \red{I}ndex (none of the others) \\
N    & {\tt 1:nI+1,1:nJ+1,1:nK+1}      &  \red{N}odes (of grid cells) \\
P    & {\tt 1:nProc}                   &  \red{P}rocessors \\
Q    & {\tt 1:4}                       &  \red{Q}uadrants for four finer neighbors\\
S    & {\tt 1:2}                       &  \red{S}ides, two ends \\
V    & {\tt 1:nVar}                    &  \red{V}ariables (state, plot...) \\
W    & {\tt 1:nWave}                   &  \red{W}aves \\
X    & {\tt 1:nI+1,1:nJ,1:nK}          &  \red{X} faces (of grid cells) \\
Y    & {\tt 1:nI,1:nJ+1,1:nK}          &  \red{Y} faces \\
Z    & {\tt 1:nI,1:nJ,1:nK+1}          &  \red{Z} faces
\end{tabular}
\end{center}
\label{tab:index}
\end{table}
