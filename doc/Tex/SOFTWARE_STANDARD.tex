...
\begin{itemize}
\item General guide lines for software development
\item Unit and functionality tests
\item Guide lines for use of object oriented features
\item Guide lines for formatting source code
\item Guide lines for documenting source code
\end{itemize}
The rest of this document will discuss these items in some detail.

Most of the guide lines are very general and apply to all kinds of 
...
Many of the guide lines may seem self-evident to many developers. That's good
news. On the other hand we know from experience that developers have radically
different approaches to software development. To be on the safe side 
...
\section{General guide-lines for software development}
...
\end{itemize}
Our experience shows that we should maintain a single branch of our software. 
...
changes in the source code relative to the previous day.
...
meaningful solutions (due to the coarsness of grids and small number 
...
This proceedure guarantees portability of the code and 
platform independence of the solution. It also verifies
that future development does not introduce unintended changes in the 
...
double precision accuracy, and the refeerence solution should be written
...
\section{Guide lines for the use of Object Oriented features 
         \label{sec:object}}
...
development.
Here we provide some guide lines on the use of some of the OO concepts 
in our software development effort. These guide lines are based 
on the experience gained in the past few years developing the SWMF.
...
For example the module ModLinerSolver may contain various methods to 
...
contains a line
\begin{verbatim}
call linear_solver(x, y, b)
\end{verbatim}
one tends to search for {\tt subroutine linear\_solver} in the source code.
...
Most scientific codes were written in the old Fortran style as a collection
of subroutines and functions. As we are gradually moving towards the object 
...
instead of using it from the module. But often one has to change the 
...
Automatic arrays another nice feature of Fortran 90. Unfortunately we found
...
and it results in undreadable and unreliable software.
...
\section{Guide lines for formatting source code}
...
\item Indentation follows the rules used by the EMACS editor. One can
      ...
      EMACS editor.
\item Comments are written with normal Engligh capitalization. All upper
...
\item Blanks are used to make the source code more readable. In particular
      ...
      by a single space. The $=$, $+$ and $-$ and the logical relations 
...
The software framework consist of the core and the components.
...
names.
Here {\it variable name} means any constant or variable, while 
...
{\bf guide lines} with respect to the choices made within 
...
data name that does not conform with the guide-lines discussed next.

\subsection{Guide lines on choosing data names and type}
...
\subsection{Guide lines on the use of abbreviations and acronyms}
...
So here are some guide lines on the use of abbreviations and acronyms
\begin{itemize}
...
\begin{itemize}
\item Procedure and module names of a model must start with a unique identifier
      of the component.
...
a name conflict. Also note that a model can use arbitrary input/output 
...
\end{verbatim}
Function names should describe the tyep and meaning of the returned value.
...
\begin{verbatim}
  ...
  Buffer_VII(:,:)   - real buffer with one variable and two oter indexes
\end{verbatim}
