...
SWMF with all 8 components. The test consists of five stages:
\begin{enumerate}
...
\item Run all 8 components in time accurate mode restarting from the
      SC+IH and GM+IM+IE+UA solutions. A CME is initiated in SC
      using an Eruptive Event generator, and 
      the SP and RB components start from scratch.
\end{enumerate}
We provide the parameter and layout files for all five stages,
but we only describe how the final stage can be run using
the restart files saved from the previous stages.
This takes much less time than running all stages, 
and it tests the whole SWMF.

\subsubsection{Configuration and Compilation}

For sake of simplicity one can configure SWMF with all 8 components
(no Empty versions) so that all 5 stages can be done with the same
executable. 
\begin{verbatim}
...
\begin{verbatim}
  cd run
  tar xzf scih_restart.tgz
  tar xzf gmimieua_restart.tgz
\end{verbatim}
...
\item The UA component can use at most 32 processors, 
      and needs at least 8 processors.
