  ...
  subroutine user_read_inputs
    ...
    do
       ...
       case('#USERINPUTEND')
          EXIT
       case default
! esm          call stop_mpi('ERROR in ModUserCrash: unknown command='//NameCommand)
          call stop_mpi('ERROR in ModUserCrashXray2dFull: unknown command='//NameCommand)
       end select
...
! esm [
    ...
    use ModImplicit, ONLY:  iVarSemi, nVarSemi
! esm ]
...
!    if (iBlock == 1) then
!       write(6,*) 'iVarSemi, nVarSemi = ', iVarSemi, nVarSemi
!       pause
!    endif
...
! esm [
! gray          ! 166 eV xray source at LH boundary
! gray          FullPower = cRadiationNo * (2.1e6 * (166./180.) * Si2No_V(UnitTemperature_))**4

          ! Setting the time at which the laser starts to turn off
          time_f = 2.00e-9    ! time in seconds

!          FullPower = 100. / 8.617385e-5     ! T(eV) / k_B(eV/K)
!          FullPower = 166. / 8.617385e-5     ! T(eV) / k_B(eV/K)
!          FullPower = 167. / 8.617385e-5     ! T(eV) / k_B(eV/K)
          FullPower = 170. / 8.617385e-5     ! T(eV) / k_B(eV/K)
!          FullPower = 202. / 8.617385e-5     ! T(eV) / k_B(eV/K)
          TBC       = FullPower
...
!             if (y_BLK(1,j,1,iBlock) <= rInnerTube) then
             if (y_BLK(1,j,1,iBlock) <= 400.0) then
                ...
                if ((Time_Simulation > 0.0).and.(Time_Simulation < 0.1e-9)) then
                   !  Laser turning on
! gray                   StateSemi_VGB(iEradImpl,-1:0,j,:,iBlock) = &
! gray                        ((FullPower - FullPower*1e-6)/0.1e-09)*Time_Simulation + &
! gray                        FullPower*1e-6

                   TBC = ((FullPower - FullPower*1e-6)/0.1e-09)*Time_Simulation + &
                        FullPower*1e-6

                   do iWave = 1, nWave
                      call get_energy_g_from_temperature(iWave,TBC,EgSI=EgSi)
                      State_VGB(WaveFirst_+iWave-1,-1:0,j,:,iBlock) = &
                           EgSi*Si2No_V(UnitEnergyDens_)
                   enddo

                elseif ((Time_Simulation >= 0.1e-9).and. &
                     (Time_Simulation <= time_f)) then
                   !  Laser at on full blast
! gray                   StateSemi_VGB(iEradImpl,-1:0,j,:,iBlock) = FullPower
                   ! Multigroup BC.
                ...
                elseif ((Time_Simulation > time_f).and. &
                     (Time_Simulation <= time_f+0.1e-9)) then
                   !  Laser shutting down
!                   StateSemi_VGB(iEradImpl,-1:0,j,:,iBlock) = &
!                        ((-FullPower + FullPower*1e-6)/0.1e-09)*Time_Simulation + &
!                        (11.*FullPower - 1.0e-5*FullPower)
! gray                   StateSemi_VGB(iEradImpl,-1:0,j,:,iBlock) = &
! gray                        ((-FullPower + FullPower*1e-6)/0.1e-09)*Time_Simulation + &
! gray                        FullPower*((1.0+10.*time_f) - 1.0e-5*time_f)

!test                   TBC = ((-FullPower + FullPower*1e-6)/0.1e-09)*Time_Simulation + &
!test                        (11.*FullPower - 1.0e-5*FullPower)

!test                   do iWave = 1, nWave
!test                      call get_energy_g_from_temperature(iWave,TBC,EgSI=EgSi)
!test                      EgSi = ((-EgSi + EgSi*1e-6)/0.1e-09)*Time_Simulation + &
!test                             EgSi*((1.0+10.*time_f) - 1.0e-5*time_f)
!test                      State_VGB(WaveFirst_+iWave-1,-1:0,j,:,iBlock) = &
!test                           EgSi*Si2No_V(UnitEnergyDens_)
!test                   enddo


                   do iWave = 1, nWave
                      State_VGB(WaveFirst_+iWave-1,0,j,:,iBlock) = &
                           State_VGB(WaveFirst_+iWave-1,1,j,:,iBlock) 
                      State_VGB(WaveFirst_+iWave-1,-1,j,:,iBlock) = &
                           State_VGB(WaveFirst_+iWave-1,1,j,:,iBlock) 
                   enddo
                   ...
                   ! Laser off (but BC set to 1.0e-6 of full power
!                  StateSemi_VGB(iEradImpl,-1:1,:,:,iBlock) = FullPower * 1.0e-6

                   !  After 1.1 ns, lase is off; use flat BCs.
! gray                   StateSemi_VGB(iEradImpl,0,j,:,iBlock) = &
! gray                        StateSemi_VGB(iEradImpl,1,j,:,iBlock) 
! gray                   StateSemi_VGB(iEradImpl,-1,j,:,iBlock) = &
! gray                        StateSemi_VGB(iEradImpl,1,j,:,iBlock) 
...
! gray                StateSemi_VGB(iEradImpl,0,j,:,iBlock) = &
! gray                     cRadiationNo*(0.0259)**4
! gray                StateSemi_VGB(iEradImpl,-1,j,:,iBlock) = &
! gray                     StateSemi_VGB(iEradImpl,1,j,:,iBlock) 

                do iWave = 1, nWave
                   call get_energy_g_from_temperature(iWave, & 
                                                      0.0259/8.617385e-5, EgSI=EgSi)
                   State_VGB(WaveFirst_+iWave-1,-1:0,j,:,iBlock) = &
                        EgSi*Si2No_V(UnitEnergyDens_)
                enddo
             ...
             do j = 1, nJ
                ...
                if (y_BLK(1,j,1,iBlock) <= 400.0) then
                   ...
                   if ((Time_Simulation > 0.0).and.(Time_Simulation < 0.1e-9)) then
                      ...
                      TBC = ((FullPower - FullPower*1e-6)/0.1e-09)* &
                           Time_Simulation + FullPower*1e-6

!                      write(6,*) 'iVarSemi = ', iVarSemi

                      call get_energy_g_from_temperature(iVarSemi-1,TBC,EgSI=EgSi)
                      StateSemi_VGB(1,-1:0,j,:,iBlock) = &
                           EgSi*Si2No_V(UnitEnergyDens_)
                   ...
                   !  Laser shutting down
                      TBC = ((-FullPower + FullPower*1e-6)/0.1e-09)* &
                           Time_Simulation + (11.*FullPower - 1.0e-5*FullPower)
...
!                   enddo
                      ...
                      !  After 1.1 ns, laser is off; use flat BCs.
                      StateSemi_VGB(1,0,j,:,iBlock) = &
                           StateSemi_VGB(1,1,j,:,iBlock) 
                      StateSemi_VGB(1,-1,j,:,iBlock) = &
                           StateSemi_VGB(1,1,j,:,iBlock) 
                   endif
...
! esm [
    ...
    real    :: xPl = 0.0                        ! [um]
    real    :: yPl                              ! [um]
    real    :: dist, distBe, distXe, distPl     ! [um]
    real    :: DxPl
    real    :: Tr = 0.0259                      ! [eV?]
!    real    :: TeSi = 300.                      ! [K]
    real    :: TeSiOut                          ! [K]
    real    :: EinternalSiOut = 1.
    real    :: PressureSiOut  = 1.

    real    :: xmin, xAu, xAy1, xAy2
...
!test
       State_VGB(LevelAu_,i,j,k,iBlock) = 0.
!test

!       write(6,*) 'rho = ', i,j,k,iBlock,State_VGB(Rho_,i,j,k,iBlock)
...
!         Set material as Be.
          ...
          distBe = abs(xBe - x)
          distPl = abs(yPl - y)
          dist = min(distBe, distPl)
          State_VGB(LevelBe_,i,j,k,iBlock) = distBe
          State_VGB(LevelXe_,i,j,k,iBlock) = -distBe
!4_mat    State_VGB(LevelPl_,i,j,k,iBlock) = -sqrt(distPl**2 + (distBe+xAu)**2)
          State_VGB(LevelPl_,i,j,k,iBlock) = -sqrt(distPl**2 + (xAy1-xBe+distBe)**2)
          State_VGB(LevelAu_,i,j,k,iBlock) = -sqrt(distBe**2 + distPl**2)
          State_VGB(LevelAy_,i,j,k,iBlock) = -sqrt((xAu-x)**2 + distPl**2)
...
! pl atm test [
          ...
          dist = distBe
          State_VGB(LevelBe_,i,j,k,iBlock) = -distBe
          State_VGB(LevelXe_,i,j,k,iBlock) = -(distBe+xBe)
          State_VGB(LevelPl_,i,j,k,iBlock) = distBe
          State_VGB(LevelAu_,i,j,k,iBlock) = -sqrt((xBe-x)**2 + distPl**2)
          State_VGB(LevelAy_,i,j,k,iBlock) = -sqrt((xAu-x)**2 + distPl**2)
! pl atm test ]


!"normal"    State_VGB(Rho_,i,j,k,iBlock) = 0.0065
             State_VGB(Rho_,i,j,k,iBlock) = 0.0065
!test        State_VGB(Rho_,i,j,k,iBlock) = 0.00065

!          write(6,*) x, y, z, State_VGB(Rho_,i,j,k,iBlock)
!          pause
...
! pl atm test [
          distBe = min(abs(xBe - x), x)
          distPl = abs(yPl - y)
          State_VGB(LevelBe_,i,j,k,iBlock) = distBe
          State_VGB(LevelXe_,i,j,k,iBlock) = -abs(xBe - x)
          State_VGB(LevelPl_,i,j,k,iBlock) = -x
          State_VGB(LevelAu_,i,j,k,iBlock) = -sqrt(distBe**2 + distPl**2)
          State_VGB(LevelAy_,i,j,k,iBlock) = -sqrt((xAu-x)**2 + distPl**2)
! pl atm test ]

          State_VGB(Rho_,i,j,k,iBlock) = 1.85
!!          State_VGB(Rho_,i,j,k,iBlock) = 0.45

       elseif ((x >= xBe).and.(x < 4000.).and.(y < rInnerTube)) then
!         ...in Xe
          ...
          dist = min(distBe, distPl)
          State_VGB(LevelBe_,i,j,k,iBlock) = -distBe
          State_VGB(LevelXe_,i,j,k,iBlock) =  dist
          if (x <= xAu) then
!4_mat       State_VGB(LevelPl_,i,j,k,iBlock) = -sqrt(distPl**2 + (xAu-x)**2)
             State_VGB(LevelPl_,i,j,k,iBlock) = -sqrt(distPl**2 + (xAy1-x)**2)
             State_VGB(LevelAu_,i,j,k,iBlock) = -abs(distPl)
             State_VGB(LevelAy_,i,j,k,iBlock) = -abs(sqrt((xAu-x)**2 + distPl**2))

! pl atm test [
             State_VGB(LevelPl_,i,j,k,iBlock) = &
                  -min( sqrt(distPl**2 + (xAy1-x)**2), abs(x))
! pl atm test ]

          elseif ((x > xAu).and.(x <= xAy1)) then
!4_mat       State_VGB(LevelPl_,i,j,k,iBlock) = -abs(distPl)
             State_VGB(LevelPl_,i,j,k,iBlock) = -sqrt(distPl**2 + (xAy1-x)**2)
             State_VGB(LevelAu_,i,j,k,iBlock) = -sqrt(distPl**2 + (xAu-x)**2)
             State_VGB(LevelAy_,i,j,k,iBlock) = -distPl
...
! pl atm test ]
          ...
          elseif ((x > xAy1).and.(x <= xAy2)) then
             State_VGB(LevelPl_,i,j,k,iBlock) = -abs(distPl)             
             State_VGB(LevelAu_,i,j,k,iBlock) = -sqrt(distPl**2 + (xAu-x)**2)
             State_VGB(LevelAy_,i,j,k,iBlock) = &
                  -min(distPl+(rOuterTube-rInnerTube), &
                       sqrt((x-xAy1)**2 + distPl**2))
! pl atm test [
             State_VGB(LevelPl_,i,j,k,iBlock) = &
                  -min( abs(distPl), abs(x) )
! pl atm test ]
             ...
             State_VGB(LevelAu_,i,j,k,iBlock) = -sqrt(distPl**2 + (xAu-x)**2)
             State_VGB(LevelAy_,i,j,k,iBlock) = &
                  -sqrt((distPl+(rOuterTube-rInnerTube))**2 + (x-xAy2)**2)
...
! pl atm test ]
       ...
       elseif ((y >= rInnerTube).and.(y <= rOuterTube)) then
          ...
          if ((x >= xmin).and.(x < 0.)) then
             ...
             State_VGB(LevelBe_,i,j,k,iBlock) = distBe
             State_VGB(LevelXe_,i,j,k,iBlock) = -sqrt(distBe**2+distPl**2)
!4_mat       State_VGB(LevelPl_,i,j,k,iBlock) = -(distBe+xAu)
...
! pl atm test [
             State_VGB(LevelBe_,i,j,k,iBlock) = -abs(x)
             State_VGB(LevelXe_,i,j,k,iBlock) = -sqrt(distBe**2+distPl**2)
             State_VGB(LevelPl_,i,j,k,iBlock) = abs(x)
             State_VGB(LevelAu_,i,j,k,iBlock) = -(abs(x)+xBe)
             State_VGB(LevelAy_,i,j,k,iBlock) = -(abs(x)+xAu)
! pl atm test ]
...
!test        State_VGB(Rho_,i,j,k,iBlock) = 0.00065
          ...
          elseif ((x >= 0.).and.(x < xBe)) then
             ...
             State_VGB(LevelAu_,i,j,k,iBlock) = -distBe
...
! pl atm test ]
          ...
          elseif ((x >= xBe).and.(x < xAu)) then
             ...
             State_VGB(LevelBe_,i,j,k,iBlock) = -distBe
             State_VGB(LevelXe_,i,j,k,iBlock) = -distPl
!4_mat       State_VGB(LevelPl_,i,j,k,iBlock) = -(xAu-x)
             State_VGB(LevelPl_,i,j,k,iBlock) = -(xAy1-x)
             State_VGB(LevelAu_,i,j,k,iBlock) = &
                  min( min(abs(distPl), (xAu-x)), (x-xBe) )
             State_VGB(LevelAy_,i,j,k,iBlock) = -(xAu-x)
...
! pl atm test ]
             ...
             State_VGB(Rho_,i,j,k,iBlock) = 20.
          ...
          elseif ((x > xAu).and.(x <= xAy1)) then
             ...
             State_VGB(LevelAy_,i,j,k,iBlock) = &
                  min( min(abs(distPl), abs(xAy1-x)), abs(x-xAu) )
...
! pl atm test ]
             ...
             State_VGB(Rho_,i,j,k,iBlock) = 1.13
          ...
          elseif ((x > xAy1).and.(x <= xAy2)) then
             ...
             State_VGB(LevelXe_,i,j,k,iBlock) = -distPl
             State_VGB(LevelPl_,i,j,k,iBlock) = &
                  min( min(abs(distPl), abs(xAy2-x)), abs(x-xAy1) )
             State_VGB(LevelAu_,i,j,k,iBlock) = -(x-xAu)
             State_VGB(LevelAy_,i,j,k,iBlock) = &
                  -min(abs(rOuterTube-y), abs(x-xAy1))

             State_VGB(Rho_,i,j,k,iBlock) = 1.43
             ...
             State_VGB(LevelXe_,i,j,k,iBlock) = -distPl
             State_VGB(LevelPl_,i,j,k,iBlock) = &
                  min(abs(distPl), sqrt((rOuterTube-y)**2 + (x-xAy2)**2))
             State_VGB(LevelAu_,i,j,k,iBlock) = -(x-xAu)
             State_VGB(LevelAy_,i,j,k,iBlock) = &
       ...
       elseif (y > rOuterTube) then
          ...
          if ((x >= xmin).and.(x < 0.)) then
             ! Be atmosphere
             State_VGB(LevelBe_,i,j,k,iBlock) = distBe
             State_VGB(LevelXe_,i,j,k,iBlock) = -sqrt(distPl**2+distBe**2)
             State_VGB(LevelPl_,i,j,k,iBlock) = &
...
! pl atm test [
             State_VGB(LevelPl_,i,j,k,iBlock) =  &
                  -min( min( xAy2-x, sqrt( (xAy1-x)**2 + (y-rOuterTube)**2 ) ), &
                        abs(x) )
! pl atm test ]
          ...
          elseif ((x >= xAu).and.(x < xAy1)) then
             ...
             State_VGB(LevelAu_,i,j,k,iBlock) = -(x-xAu)
             State_VGB(LevelAy_,i,j,k,iBlock) = &
                  min( min( distPl, (x-xAu) ), &  
                       sqrt( (xAy1-x)**2 + (y-rOuterTube)**2 ) )

! pl atm test [
             State_VGB(LevelPl_,i,j,k,iBlock) = &
                  -min( sqrt( (xAy1-x)**2 + (y-rOuterTube)**2 ), &
                        abs(x) )
! pl atm test ]
          ...
          elseif ((x >= xAy1).and.(x < xAy2)) then
             ...
             State_VGB(LevelAu_,i,j,k,iBlock) = -(x-xAu)
             State_VGB(LevelAy_,i,j,k,iBlock) = &
                  min( min( (y-rOuterTube), (x-xAu) ), (xAy2-x) )

! pl atm test [
             State_VGB(LevelPl_,i,j,k,iBlock) = &
                  -min( min( (xAy2-x), (y-rOuterTube) ), &
                        abs(x) )
! pl atm test ]
             ...
             State_VGB(LevelBe_,i,j,k,iBlock) = -distBe
             State_VGB(LevelXe_,i,j,k,iBlock) = -distPl
             State_VGB(LevelPl_,i,j,k,iBlock) = min(abs(distPl), (x-xAy2))
             State_VGB(LevelAu_,i,j,k,iBlock) = -(x-xAu)
             State_VGB(LevelAy_,i,j,k,iBlock) = -(x-xAy2)

! pl atm test [
             State_VGB(LevelPl_,i,j,k,iBlock) = &
                  min( min(abs(distPl), (x-xAy2)), &
                       abs(x) )
! pl atm test ]
...
! esm [
!       write(6,*) 'user_set_ics: TeSiIn = ', TeSiIn
!       pause
! esm ]

!      Set initial temperature.

       TeSi = 2.1e6 *(0.025/180.) ! * Si2No_V(UnitTemperature_)
!test       if (x < 0.) then
!test          TeSi = 2.1e6 *(180./180.) ! * Si2No_V(UnitTemperature_)
!test       else
!test          TeSi = 2.1e6 *(0.025/180.) ! * Si2No_V(UnitTemperature_)
!test       endif
...
!!            TeOut=TeSi, &
!!            PressureOut=PressureSiOut)

!!       State_VGB(p_,i,j,k,iBlock) = PressureSiOut*Si2No_V(UnitP_)



! user_material_properties(State_V, i, j, k, iBlock, iDir, &
...
!       State_VGB(p_,i,j,k,iBlock) =  1.0e-10
!       EinternalSi = 1.0e-10
...
!          TrSi = Tr*No2Si_V(UnitTemperature_)
          TrSi = 11600.
!test          if (x < 0.) then
!test             TrSi = 180. / 8.617385e-5     ! T(eV) / k_B(eV/K)
!test          else
!test             TrSi = 11600.
!test          endif

!          write(6,*) 'BEFORE TrSi = ', TrSi
          do iWave = 1, nWave
...
!             write(6,*) 'TrSi = ', TrSi
!             write(6,*) 'EgSi = ', EgSi
!             write(6,*) 'State_VGB(WaveFirst_+iWave-1,i,j,k,iBlock) = ', &
!                  iWave, State_VGB(WaveFirst_+iWave-1,i,j,k,iBlock)
!             pause
...
!esm          TeSi = Te_G(i,j,k)*No2Si_V(UnitTemperature_)
!esm [
          TeSi = 2.1e6 *(0.025/180.) ! * Si2No_V(UnitTemperature_)
          Te_G(i,j,k) = TeSi/No2Si_V(UnitTemperature_)
          Ti_G(i,j,k) = TeSi/No2Si_V(UnitTemperature_)
!esm ]
          call user_material_properties(State_VGB(:,i,j,k,iBlock), &
               i, j, k, iBlock, TeIn=TeSi, &
...
!esm             State_VGB(p_,i,j,k,iBlock)  = Natomic*Ti_G(i,j,k)
!esm [
             State_VGB(p_,i,j,k,iBlock)  = Natomic*TeSi/No2Si_V(UnitTemperature_)
!esm ]
          end if
...
! esm [
       State_VGB(RhoUz_,1:nI,1:nJ,1:nK,iBlock) =  0.0
! esm ]
