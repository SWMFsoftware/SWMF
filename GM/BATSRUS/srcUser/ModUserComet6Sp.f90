...
! write_plot_commonsComet.f90, into write_plot_commons.f90, ModPlanetConstComet.f90
! into share/Library/ModPlanetConst.f90.
!==============================================================================
...
!convert to real solar distance value
    IoniRate_I = IoniRate_I*ionization_rate*cE6
...
!define neutral density array, necessary for special neutral distribution types only
    NNeu_BLK(:,:,:,globalBLK,H2O_) = Qprod * &	!!!Number Density!!!
        ...
        (cFour*cPi*Unr*R_BLK(0:nI+1,0:nJ+1,0:nK+1,globalBLK)**2*No2Si_V(UnitX_)**2)
	  ...
	  if ( jpattern == 22 ) then		!uniform estimated H density
       NNeu_BLK(:,:,:,globalBLK,H_) = Qprod * &
            ...
            No2Si_V(UnitX_)/lambda/12.)/ &
        	  (cFour*cPi*Unr*R_BLK(0:nI+1,0:nJ+1,0:nK+1,globalBLK)**2* &
            No2Si_V(UnitX_)**2*cTwo*cThree)
...
!The following is for the special impact ionization and recombination [gombosi96].
       Rkm  = R_BLK(i,j,k,globalBLK)*No2Si_V(UnitX_)/cE3	!km unit
       logR = log10(Rkm)
       if (jpattern == 10 .or. jpattern == 11 ) then
          if (rkm >= 5000. .and. rkm < 10000.) then
	     fi = 1.0+0.77*log(rkm/5000.)
          elseif (rkm >= 10000. .and. rkm < 50000.) then
             fi = 1.5-0.31067*log(rkm/10000.)
          else
	     fi = cOne
          endif
       else
	  ...
	  if ( Rkm <= 1584.893 ) then
	     ...
	     Te = cTen**( 1.143*logR-1.667 )
          else if ( Rkm <= 1.e4 ) then
	     Te = cTen**( 10.965*logR-39.3725 )
          else if ( Rkm <= 1.e5 ) then
	     Te = cTen**( .5135*logR+2.4325 )
          else
	     Te = 1.e5
          endif
       else
...
!define recombination terms ion by ion
       ...
       AlphaN_I = AlphaN_I*totalNumRho/cE6*No2Si_V(UnitN_)*No2Si_V(UnitT_)	!nomalize
...
!calculate Source_VC
	  ...
	  Source_VC(rhoU_+m,i,j,k) = Source_VC(rhoU_+m,i,j,k) + &
       ...
       Source_VC(p_,i,j,k) = Source_VC(p_,i,j,k) + &
		cThird*(totalSourceRho-Source_H3Op+collisn)* &
...
if((n_step ==1 .or. n_step ==4000) .and. iproc==PROCTest .and. &
globalBLK==BlkTest .and. i==iTest .and. j==jTest .and. k==kTest ) then
write(*,*) ' totalnumrho= ', totalNumRho, &
'totalSourceRho = ', totalSourceRho, 'totalNumLoss =', totalNumLoss, &
'usqr= ', usqr, 'unsqr = ', unsqr, 'Src_I=', Source_I, &
'Los_I=', Loss_I, 'Loss_H3Op=', Loss_H3Op, 'Source_H3Op', Source_H3Op
write(*,*) 'totalLossrho =', totalLossRho, &
'AlphaN_I=', AlphaN_I, 'inirate', ionization_rate, 'IoniRate_I,fi=', &
IoniRate_I, fi
...
if((n_step==1 .or. n_step==4000) .and. iproc==PROCTest .and. &
globalBLK == BlkTest) then
write(*,*) '***source = ', Source_VC(:,iTest,jTest,kTest), '**xyz = ', &
X_BLK(iTest,jTest,kTest,BlkTest), Y_BLK(iTest,jTest,kTest,BlkTest), &
Z_BLK(iTest,jTest,kTest,BlkTest), &
'state ===', State_VGB(:,iTest,jTest,kTest,BlkTest), 'kin,unr = ', kin, unr
write(*,*) 'neutral =', NNeu_BLK(iTest,jTest,kTest,BlkTest,:), &
UNeu_BLK(iTest,jTest,kTest,BlkTest,:)
write(*,*) 'normalize =', No2SI_V(:)
endif
  ...
  subroutine user_update_states(iStage,iBlock)
    ...
    !/
! The following is now moved to Update_States_MHD: DoReplaceDensity
!    if ( UseMultiSpecies ) then
!       do k=1,nK ;   do j=1,nJ ;    do i=1,nI
