...
!!$    write(*,*)' Source_VC(P)=',Source_VC(p_,iTest,jTest,kTest)
    ...
    do k = 1, nK ;   do j = 1, nJ ;  do i = 1, nI
       ...
       if (kTi_dim >= 1.0e4.and.kTi_dim <= 1.0e7)then
          if(kTi_dim <= 1.0e5)then
             h=int(kTi_dim/1.04 -1.0)
          else if(kTi_dim <= 1.0e6)then
             h=int(kTi_dim/1.0e5+9.0)
          else if(kTi_dim <= 5.0e6)then
             h=int(kTi_dim/1.0e6+19.0)
          else if(kTi_dim <= 7.0e6)then
...
!!$
  ...
  subroutine enceladus_input(iBlock)
    ...
    logical:: DoTest
    character(len=*), parameter:: NameSub = 'enceladus_input'
    !--------------------------------------------------------------------------
    call test_start(NameSub, DoTest, iBlock)
