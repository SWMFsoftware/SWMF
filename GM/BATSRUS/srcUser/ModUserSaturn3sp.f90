...
Module ModUser
  ...
  character (len=*), parameter :: &
       NameUserModule = 'Saturn Magnetosphere 3 Species (Saturn3sp), Hansen, May, 2007'
  ...
  real, dimension(1:nI,1:nJ,1:nK):: &
       Srho,SrhoH,SrhoH2O,SrhoN,SrhoUx,SrhoUy,SrhoUz,SBx,SBy,SBz,Sp,SE
...
contains
  ...
  use ModPhysics,   ONLY : CellState_VI
    ...
    ! or sets them to undefined values (fixed) so we set them here below.
    !
  ...
  subroutine user_calc_sources
    use ModAdvance, ONLY: Source_VC, rhoH_, rhoH2O_, rhoN_
    ...
    Source_VC(rhoH_   ,:,:,:) = SrhoH   + Source_VC(rhoH_   ,:,:,:)
    Source_VC(rhoH2O_ ,:,:,:) = SrhoH2O + Source_VC(rhoH2O_ ,:,:,:)
    Source_VC(rhoN_   ,:,:,:) = SrhoN   + Source_VC(rhoN_   ,:,:,:)
       ...
       do k = 1, nK
          do j = 1, nJ
  		 ...
  		 ! rate.
                   ...
                   ! testing only - if not zero then change the equations below.
                   Alpha_rec = 0.0
                   CXsource = 0.0
                   
                   ! now load the source terms into the right hand side.  Do rho as with
                   ! single species, being the total mass.  Do the SrhoH2O since this is 
                   ! just the water part.
  

  !!!!! Check the rhoH2O_ in the recombination term

                   Srho(i,j,k)   = Srho(i,j,k)                                        &  
                                   + (rhodot-Alpha_rec*State_VGB(rhoH2O_,i,j,k,globalBLK))  
                   SrhoH2O(i,j,k) = SrhoH2O(i,j,k)                                        &  
                                   + (rhodot-Alpha_rec*State_VGB(rhoH2O_,i,j,k,globalBLK))  
  		 ...
  		 !
                   ...
                   ! now load the source terms into the right hand side.  Do rho as with
                   ! single species, being the total mass.  Do the SrhoN since this is 
                   ! just the water part.
   
  !!!! Check the rhoN_ in the recombination term.

                   Srho(i,j,k)   = Srho(i,j,k)   + (rhodot - Lterm*State_VGB(rhoN_,i,j,k,globalBLK))
                   SrhoN(i,j,k)  = SrhoN(i,j,k)   + (rhodot - Lterm*State_VGB(rhoN_,i,j,k,globalBLK))
