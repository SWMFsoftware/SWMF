...
//  VT_traceoff();
  ...
  PIC::ParticleWeightTimeStep::maxReferenceInjectedParticleNumber=20; //00; //*10;
  PIC::RequiredSampleLength=1700; //00; //0; //0;
  ...
  if (PIC::ThisThread==0) {
    Moon::SourceProcesses::PhotonStimulatedDesorption::EnergyDistribution.fPrintCumulativeDistributionFunction("CumulativeEnergyDistribution-PSD.dat");
    Moon::SourceProcesses::PhotonStimulatedDesorption::EnergyDistribution.fPrintDistributionFunction("EnergyDistribution-PSD.dat");
...
//    cout << Moon::SourceProcesses::PhotonStimulatedDesorption::EnergyDistribution.DistributeVariable() << endl;
  ...
  //register the sphere
  {
    ...
    Moon::SourceProcesses::PhotonStimulatedDesorption::SurfaceInjectionDistribution.SetLimits(0,PIC::BC::InternalBoundary::Sphere::TotalSurfaceElementNumber-1,
        Moon::SourceProcesses::PhotonStimulatedDesorption::GetSurfaceElementSodiumProductionRate);


  #if _EXOSPHERE_SOURCE__THERMAL_DESORPTION_ == _EXOSPHERE_SOURCE__ON_
    Moon::SourceProcesses::ThermalDesorption::SurfaceInjectionDistribution.SetLimits(0,PIC::BC::InternalBoundary::Sphere::TotalSurfaceElementNumber-1,
        Moon::SourceProcesses::ThermalDesorption::GetSurfaceElementSodiumProductionRate);
  #endif

  #if _EXOSPHERE_SOURCE__SOLAR_WIND_SPUTTERING_ == _EXOSPHERE_SOURCE__ON_
    Moon::SourceProcesses::SolarWindSputtering::SurfaceInjectionDistribution.SetLimits(0,PIC::BC::InternalBoundary::Sphere::TotalSurfaceElementNumber-1,
        Moon::SourceProcesses::SolarWindSputtering::GetSurfaceElementSodiumProductionRate);
  #endif
...
//  int LastDataOutputFileNumber=-1;
  ...
  for (el=0;el<PIC::BC::InternalBoundary::Sphere::TotalSurfaceElementNumber;el++) {
    ...
    MPI_Bcast(Moon::Planet->SolarWindSurfaceFlux+el,1,MPI_DOUBLE,node->Thread,MPI_COMM_WORLD);
  }
...
//  VT_TRACER("main");
  ...
  for (long int niter=0;niter<100000001;niter++) {
    ...
    spkezr_c("Earth",Moon::OrbitalMotion::et+0.5*PIC::ParticleWeightTimeStep::GlobalTimeStep[_NA_SPEC_],Moon::SO_FRAME,"none","MOON",StateEarth_SO,&lt);
    spkezr_c("Earth",Moon::OrbitalMotion::et+0.5*PIC::ParticleWeightTimeStep::GlobalTimeStep[_NA_SPEC_],"MSGR_HCI","none","MOON",StateEarth_HCI,&lt);

    //calculate lunar velocity in an itertial frame, which have dirtectional vectors that coinsides with that of LSO
    sxform_c("MSGR_HCI","LSO",Moon::OrbitalMotion::et+0.5*PIC::ParticleWeightTimeStep::GlobalTimeStep[_NA_SPEC_],HCI_to_MSO_TransformationMartix);
    spkezr_c("Moon",Moon::OrbitalMotion::et+0.5*PIC::ParticleWeightTimeStep::GlobalTimeStep[_NA_SPEC_],"MSGR_HCI","none","SUN",StateMiddle,&lt);


    Moon::OrbitalMotion::et+=PIC::ParticleWeightTimeStep::GlobalTimeStep[_NA_SPEC_];
    ...
    sxform_c("MSGR_HCI","LSO",Moon::OrbitalMotion::et-PIC::ParticleWeightTimeStep::GlobalTimeStep[_NA_SPEC_],fmatrix);
    ...
    Moon::RotationRate_SO_FROZEN=acos(SunPointingDirectionEnd_MSO_FROZEN[0])/PIC::ParticleWeightTimeStep::GlobalTimeStep[_NA_SPEC_];
    ...
    sxform_c("LSO","MSGR_HCI",Moon::OrbitalMotion::et-PIC::ParticleWeightTimeStep::GlobalTimeStep[_NA_SPEC_],T1);
