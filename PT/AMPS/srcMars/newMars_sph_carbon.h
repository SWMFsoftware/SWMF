...
//    ForwardCollisionCrossSection.AddProfile(3.1*eV2J,_KHANCHERKO_LENGTH_,_KHANCHERKO_DATA_);
    ...
    ReadMTGCM();
      ...
      SphericalSamplingMesh.init(135.0E3+_RADIUS_(_TARGET_),1135.0e3+_RADIUS_(_TARGET_),100,50,25,nSampledFieldVariablesSphericalVolumeMesh,PrintVariableListSphericalVolumeMesh,GetSampledVariableSphericalSamplingMesh);
...
*/	
	...
	//	double COpScaleHeight=102.1E3; //Solar Max
		...
		for (idim=0;idim<DIM;idim++) {vectop[idim] = (_RADIUS_(_TARGET_)+200E3)*(x[idim]/r);}
		Altitude = r-_RADIUS_(_TARGET_);	
		

		#if DR == Rosen //Rosen [1998]
			if (Altitude<200E3){production_rate=2.75E-7*pow(300/Te.Interpolate(x),0.55)*COp.Interpolate(x)*E.Interpolate(x);}
			else if (Altitude>=200E3){production_rate=2.75E-7*pow(300/Te.Interpolate(x),0.55)*E.Interpolate(x)*(COp.Interpolate(vectop)*exp(-(Altitude-200E3)/COpScaleHeight));}
		#endif
		#if DR == Cloutier //Cloutier and Daniell [1979]
			if (Altitude<200E3){production_rate=6.47E-7*pow(300/Te.Interpolate(x),0.53)*COp.Interpolate(x)*E.Interpolate(x);}
			else if (Altitude>=200E3){production_rate=6.47E-7*pow(300/Te.Interpolate(x),0.53)*E.Interpolate(x)*(COp.Interpolate((vectop)*exp(-(Altitude-200E3)/COpScaleHeight));}
		#endif
		#if DR == Mitchell //Mitchell and Hus [1985]
			if (Altitude<200E3){production_rate=2.0E-7*pow(300/Te.Interpolate(x),0.48)*COp.Interpolate(x)*E.Interpolate(x);}
			else if (Altitude>=200E3){production_rate=2.0E-7*pow(300/Te.Interpolate(x),0.48)*E.Interpolate(x)*(COp.Interpolate((vectop)*exp(-(Altitude-200E3)/COpScaleHeight));}
		#endif
