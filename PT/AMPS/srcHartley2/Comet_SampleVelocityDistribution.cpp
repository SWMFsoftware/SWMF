/*
 * Comet_SampleVelocityDistribution.cpp
 *
...
Comet::Sampling::VelocityDistribution::cVelocitySampleBuffer *Comet::Sampling::VelocityDistribution::SampleBuffer=NULL;
int Comet::Sampling::VelocityDistribution::nTotalSampleDirections=0.0;
int Comet::Sampling::VelocityDistribution::nZenithPoints=0;


void Comet::Sampling::VelocityDistribution::Init() {
...
void Comet::Sampling::VelocityDistribution::Sampling() {
  ...
  spkezr_c("Sun",Exosphere::OrbitalMotion::et,"LSO","none","Comet",SunState,&lt);
...
//        dl=IntegrationStep2CellSizeRatio*xNode->GetCharacteristicCellSize();
        ...
        while (ptr!=-1) {
          ...
          for (idim=0,c=0.0;idim<DIM;idim++) c+=pow(x[idim]-Comet::xSun_SO[idim],2);

          for (Speed=0.0,rHeliocentric=sqrt(c),HeliocentricRadialVelocity=0.0,idim=0;idim<DIM;idim++) {
            HeliocentricRadialVelocity+=(v[idim]-vSunLSO[idim])*(x[idim]-Comet::xSun_SO[idim])/rHeliocentric;
          ...
          if (spec==_NA_SPEC_) if ( (Comet::EarthShadowCheck(x)==false) && ((x[0]>0.0)||(x[1]*x[1]+x[2]*x[2]>_RADIUS_(_MOON_)*_RADIUS_(_MOON_))) ) {
      ...
      //break the loop if the new point is within the Comet or outside of the computational domain
...
void Comet::Sampling::VelocityDistribution::OutputSampledData(int DataOutputFileNumber) {
#if _EXOSPHERE__ORBIT_CALCUALTION__MODE_ == _PIC_MODE_ON_
  ...
  if (PIC::ThisThread==0) {
    sprintf(fname,"%s/pic.Comet.VelocityDistribution.AntisolarDirection.out=%i.dat",PIC::OutputDataFileDirectory,DataOutputFileNumber);
    fout=fopen(fname,"w");
    ...
    sprintf(fname,"%s/pic.Comet.VelocityDistribution.AntisolarDirection.Map.out=%i.dat",PIC::OutputDataFileDirectory,DataOutputFileNumber);
    fMAP=fopen(fname,"w");
