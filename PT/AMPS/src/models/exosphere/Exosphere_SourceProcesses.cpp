...
double Exosphere::SourceProcesses::SolarWindSputtering::TypicalIonFluxSputteringRate(int spec) {
  ...
  res=SolarWindSputtering_Yield[spec]*Pi*pow(Exosphere::Planet->Radius,2)*Exosphere::swNumberDensity_Typical*
  ...
  res=SolarWindSputtering_UserRequestedSourceRate[spec];
  #else
