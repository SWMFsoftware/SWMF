...
/*
  ...
  //==========   DEBUG END ===================
    ...
    for (int nTimeStepSplit=0;nTimeStepSplit<5;nTimeStepSplit++) {
      int nTotalSubSteps=(1<<nTimeStepSplit);
      double dtSubStepMax=dt/nTotalSubSteps;
      double dtSubStep=0.0,dtTotalCounter=0.0,newDtSubStep;
      double dtSubStepElectronCurrent,dtSubStepIonCurrent,IonCurrentMultiplier,ElectronCurrentMultiplier,newGrainElectricCharge,newDustPotential,newElectronCurrentMultiplier,newIonCurrentMultiplier;
      double dtSubStepMaxAbsolute=dtSubStepMax;
  ...
  //      dtSubStep=min(min(min(dt-dtTotalCounter,dtSubStepMax),min(dtSubStepElectronCurrent,dtSubStepIonCurrent)),dtSubStepMaxAbsolute);
        ...
        / *--------------------------------  Evaluate the times step of the integration  ----------------------* /
 //       if (dtSubStep<0.0) dtSubStep=min(min(min(dt-dtTotalCounter,dtSubStepMax),min(dtSubStepElectronCurrent,dtSubStepIonCurrent)),dtSubStepMaxAbsolute);

        dtSubStep=10*log(0.5)*(4.0*Pi*VacuumPermittivity*GrainRadius)*(Kbol*plasmaTemperature)/ElectronCharge/electronCurrent;
        if (dtSubStep>dtSubStepMaxAbsolute) dtSubStep=dtSubStepMaxAbsolute;




        newDtSubStep=dtSubStep;



        / *-------------------------------- Time Integration of grain's charge ---------------------------* /
        while (TimeStepIntegrationFinished==false) {


        dtSubStep=newDtSubStep;
        if (dtSubStep+dtTotalCounter>=dt) dtSubStep=dt-dtTotalCounter;
        ...
        newGrainElectricCharge=GrainElectricCharge_FIRST_ORDER+(ionCurrent+electronCurrent)*dtSubStep/2.0;
        newDustPotential=newGrainElectricCharge/(4.0*Pi*VacuumPermittivity*GrainRadius);
        ...
        electronCurrent=newElectronCurrentMultiplier*basisElectronCurrent;
        ionCurrent=newIonCurrentMultiplier*basisIonCurrent;
        ...
        if (  (fabs(newElectronCurrentMultiplier-ElectronCurrentMultiplier)<=0.1*fabs(newElectronCurrentMultiplier+ElectronCurrentMultiplier)) &&
                    (fabs(newIonCurrentMultiplier-IonCurrentMultiplier)<=0.1*fabs(newIonCurrentMultiplier+IonCurrentMultiplier)) ) newDtSubStep*=2.0;


        if (  ((fabs(newElectronCurrentMultiplier-ElectronCurrentMultiplier)<=0.3*fabs(newElectronCurrentMultiplier+ElectronCurrentMultiplier)) &&
            (fabs(newIonCurrentMultiplier-IonCurrentMultiplier)<=0.3*fabs(newIonCurrentMultiplier+IonCurrentMultiplier)))  ||
            (dtSubStep<1.0E-5*dt) ) break;


        / *--------------------------- Decrease the time step if needed ----------------* /
...
//       if (dtSubStep<0.0) dtSubStep=min(min(min(dt-dtTotalCounter,dtSubStepMax),min(dtSubStepElectronCurrent,dtSubStepIonCurrent)),dtSubStepMaxAbsolute);
      ...
      / *-------------------------  Step 1 ---------------------------* /
        ...
        electronCurrent=ElectronCurrentMultiplier*basisElectronCurrent;
        ionCurrent=IonCurrentMultiplier*basisIonCurrent;
        k1=electronCurrent+ionCurrent;

      / *-------------------------  Spep 2 ---------------------------* /

      newGrainElectricCharge=GrainElectricCharge_FIRST_ORDER+k1*dtSubStep/2.0;
      newDustPotential=newGrainElectricCharge/(4.0*Pi*VacuumPermittivity*GrainRadius);
      ...
      electronCurrent=newElectronCurrentMultiplier*basisElectronCurrent;
      ionCurrent=newIonCurrentMultiplier*basisIonCurrent;
      k2=electronCurrent+ionCurrent;

      / *-------------------------  Step 3 ---------------------------* /

      newGrainElectricCharge=GrainElectricCharge_FIRST_ORDER+dtSubStep*(-k1+2.0*k2);
      newDustPotential=newGrainElectricCharge/(4.0*Pi*VacuumPermittivity*GrainRadius);
      ...
      electronCurrent=newElectronCurrentMultiplier*basisElectronCurrent;
      ionCurrent=newIonCurrentMultiplier*basisIonCurrent;
      k3=electronCurrent+ionCurrent;

      / *-------------------------  Step 3 ---------------------------* /

      newGrainElectricCharge=GrainElectricCharge_FIRST_ORDER+(k1+4.0*k2+k3)/6.0;
      newDustPotential=newGrainElectricCharge/(4.0*Pi*VacuumPermittivity*GrainRadius);
