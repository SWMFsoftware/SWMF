...
int Orbiter::GravityFieldOffset=-1;

/*
int Orbiter::Sampling::SubsolarLimbColumnIntegrals::_NA_EMISSION_5891_58A_SAMPLE_OFFSET_=-1;
int Orbiter::Sampling::SubsolarLimbColumnIntegrals::_NA_EMISSION_5897_56A_SAMPLE_OFFSET_=-1;
int Orbiter::Sampling::SubsolarLimbColumnIntegrals::_NA_COLUMN_DENSITY_OFFSET_=-1;
*/
...
void Orbiter::Init_AfterParser() {
  ...
  Orbiter::Sampling::VelocityDistribution::Init();
  PIC::Sampling::ExternalSamplingLocalVariables::RegisterSamplingRoutine(Orbiter::Sampling::VelocityDistribution::Sampling,Orbiter::Sampling::VelocityDistribution::OutputSampledData);
  ...
  Orbiter::Sampling::Kaguya::Init();
  PIC::Sampling::ExternalSamplingLocalVariables::RegisterSamplingRoutine(Sampling::SubsolarLimbColumnIntegrals::EmptyFunction,Orbiter::Sampling::Kaguya::TVIS::OutputModelData);
  */
}

int Orbiter::RequestDataBuffer(int offset) {
  int TotalDataLength;
...
void Orbiter::PrintVariableList(FILE* fout,int DataSetNumber) {
  fprintf(fout,",\"Gx\",\"Gy\",\"Gz\"");
}

void Orbiter::PrintData(FILE* fout,int DataSetNumber,CMPI_channel *pipe,int CenterNodeThread,PIC::Mesh::cDataCenterNode *CenterNode) {
  int idim;
  double t;
...
void Orbiter::Interpolate(PIC::Mesh::cDataCenterNode** InterpolationList,double *InterpolationCoeficients,int nInterpolationCoeficients,PIC::Mesh::cDataCenterNode *CenterNode) {
...
void Orbiter::GetGravityAcceleration(double *x,long int nd,cTreeNodeAMR<PIC::Mesh::cDataBlockAMR> *node) {
  register int idim;
...
double Exosphere::GetSurfaceTemeprature(double CosSubSolarAngle,double *x_LOCAL_SO_OBJECT) {
  /*
  //determine if the point on the night side of the Orbiter
  if (CosSubSolarAngle<0.0) return 100.0;

  //determine if the point is within the shadow of the Earth
  if (Orbiter::EarthShadowCheck(x_LOCAL_SO_OBJECT)==true) return 100.0;
  ...
  //determine if the point on the night side of the Orbiter
  if (CosSubSolarAngle<0.0) return minTemp[Orbiter::ndist];
  
  //return the day-side temeprature
  angle=(int) (acos(CosSubSolarAngle)*180.0/Pi);
  if(angle>89) angle=89;
  res=(SurfaceTemp[angle][Orbiter::ndist+1]>minTemp[Orbiter::ndist]) ?  SurfaceTemp[angle][Orbiter::ndist+1] : minTemp[Orbiter::ndist];
...
int Exosphere::ColumnIntegral::GetVariableList(char *vlist) {
  ...
  if (_NA_SPEC_>=0) Orbiter::Sampling::SubsolarLimbColumnIntegrals::_NA_COLUMN_DENSITY_OFFSET_=2*_NA_SPEC_;

  //sodium emission
  if (_NA_SPEC_>=0) {
    Orbiter::Sampling::SubsolarLimbColumnIntegrals::_NA_EMISSION_5891_58A_SAMPLE_OFFSET_=nVariables;
    Orbiter::Sampling::SubsolarLimbColumnIntegrals::_NA_EMISSION_5897_56A_SAMPLE_OFFSET_=nVariables+1;
...
void Exosphere::ColumnIntegral::CoulumnDensityIntegrant(double *res,int resLength,double* x,cTreeNodeAMR<PIC::Mesh::cDataBlockAMR>* node) {
  ...
  for (spec=0;spec<PIC::nTotalSpecies;spec++) {
    ...
    if (spec==_NA_SPEC_) {
      //check if the point is outside of the Orbiter's and Earth's shadows
      if ( (Orbiter::EarthShadowCheck(x)==false) && ((x[0]>0.0)||(x[1]*x[1]+x[2]*x[2]>_RADIUS_(_MOON_)*_RADIUS_(_MOON_))) ) {
...
double Exosphere::OrbitalMotion::GetTAA(SpiceDouble EphemerisTime) {
  ...
  double vOrbiter[3],xOrbiter[3],rGeocentric=0.0;
  int idim;


  spkezr_c("Orbiter",EphemerisTime,"MSGR_HCI","none","Earth",State,&ltlocal);

  for (idim=0;idim<3;idim++) {
    xOrbiter[idim]=State[idim]*1.0E3;
    vOrbiter[idim]=State[idim+3]*1.0E3;

    rGeocentric+=pow(xOrbiter[idim],2);
  }

  rGeocentric=sqrt(rGeocentric);
  Speed2=vOrbiter[0]*vOrbiter[0]+vOrbiter[1]*vOrbiter[1]+vOrbiter[2]*vOrbiter[2];
  c=xOrbiter[0]*vOrbiter[0]+xOrbiter[1]*vOrbiter[1]+xOrbiter[2]*vOrbiter[2];

  for (idim=0,absEccentricity=0.0,a=0.0;idim<3;idim++) {
    EccentricityVector[idim]=Speed2/GravitationalParameter*xOrbiter[idim] - c/GravitationalParameter*vOrbiter[idim] - xOrbiter[idim]/rGeocentric;
    absEccentricity+=EccentricityVector[idim]*EccentricityVector[idim];
    a+=EccentricityVector[idim]*xOrbiter[idim];
  }
...
long int Orbiter::InjectionBoundaryModel_Limited() {
  int spec;
  long int res=0;
...
long int Orbiter::InjectionBoundaryModel_Limited(int spec) {
  ...
  double totalProductionRate=Orbiter::GetTotalProductionRateBjornNASTRAN(spec,Sphere);
...
FluxSourceProcess[_EXOSPHERE_SOURCE__ID__USER_DEFINED__0_Bjorn_]=Orbiter::GetTotalProductionRateBjornNASTRAN(spec,Sphere);
 ...
 if (_DUST_SPEC_<=spec && spec<_DUST_SPEC_+ElectricallyChargedDust::GrainVelocityGroup::nGroups) {
   ...
   while (GrainInjectedMass>0.0) {
     ...
     flag=Orbiter::GenerateParticlePropertiesBjornNASTRAN(spec,x_SO_OBJECT,x_IAU_OBJECT,v_SO_OBJECT,v_IAU_OBJECT,sphereX0,sphereRadius,startNode,Sphere,tempParticleData);
...
double Orbiter::GetTotalProductionRateBjornNASTRAN(int spec, cInternalSphericalData* Sphere){
  return 2.0e15;
}


bool Orbiter::GenerateParticlePropertiesBjornNASTRAN(int spec, double *x_SO_OBJECT,double *x_IAU_OBJECT,double *v_SO_OBJECT,double *v_IAU_OBJECT,double *sphereX0, double sphereRadius,cTreeNodeAMR<PIC::Mesh::cDataBlockAMR>* &startNode, cInternalSphericalData* Sphere,char* tempParticleData) {
...
void Orbiter::AntiSolarDirectionColumnMap::Print(int DataOutputFileNumber) {
#if _EXOSPHERE__ORBIT_CALCUALTION__MODE_ == _PIC_MODE_ON_
  FILE *fout=NULL;
  char fname[300];
  ...
  spkezr_c("Earth",Exosphere::OrbitalMotion::et,"LSO","none","Orbiter",EarthState,&lt);
  ...
  if (PIC::ThisThread==0) {
    ...
    //open data file
    sprintf(fname,"%s/pic.Orbiter.Anti-sunwardColumnIntegrals.out=%i.dat",PIC::OutputDataFileDirectory,DataOutputFileNumber);
