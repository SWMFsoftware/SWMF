...
! This file was modified from rbe_sub_org.f90:
! (1) In subroutine readInputData, dt is read from rbe_swmf.dat
! (2) In subroutine boundary, xnd is set to xnswb
! (3) In subroutine fieldpaa, the calculation of tya is corrected
! (4) In subroutine drift, new Courant numbers is calculated from do i=1,ir
! (5) In subroutine p_result, slightly change in the format of printing hour
!     and parmod in fls file.
...
1       call timing_start('rbe_sort')
        ...
        call timing_start('rbe_integrate')
        ...
        SEARCH: do mir=1,im2-1 ! Set up arrarys at trace grids
           ...
           tya3(mir)=ss1/ra(ieq)/2.
           !!!if(ro(max(1,i),j)>0.0) &
           !!!     tya3(mir)=ss1/ro(max(1,i),j)/2.
        ...
        call timing_stop('rbe_integrate')
