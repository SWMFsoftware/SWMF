...
subroutine advance_vertical_1stage( &
  ...
  real, intent(in) :: LogNS(-1:nAlts+2,nSpeciesTotal)
  ...
  real, intent(inout) :: NewLogNS(-1:nAlts+2,nSpeciesTotal)
  ...
  real :: NS(-1:nAlts+2,nSpeciesTotal), Pressure1D(-1:nAlts+2)
  real :: Rho(-1:nAlts+2)
  ...
  real, dimension(1:nAlts,nSpeciesTotal)  :: GradLogNS, DiffLogNS
  real, dimension(1:nAlts,nSpecies)       :: GradVertVel, DiffVertVel, DivVertVel
...
! oscillating between the bottom and top of the model.
  ...
  do iAlt = 1,nAlts
     ...
     do iSpecies=nSpecies+1,nSpeciesTotal
        NewLogNS(iAlt,iSpecies) = LogNS(iAlt,iSpecies) - Dt * &
             (DivVel(iAlt) + &
             Vel_GD(iAlt,iUp_) * GradLogNS(iAlt,iSpecies) ) + &
              Dt * DiffLogNS(iAlt,iSpecies)
     enddo
...
!!!
!!!     varLeft(i) =  &
!!!          - (dalt_f(i)**2)/(4*dalt_f(i-1)*(dalt_f(i-1)+dalt_f(i))) * var(i-2) &
!!!          + (2*dalt_f(i-1)+dalt_f(i))/(4*dalt_f(i-1)) * var(i-1) &
!!!          + (2*dalt_f(i-1)+dalt_f(i))/(4*(dalt_f(i-1)+dalt_f(i))) * var(i)
!!!
!!!     varRight(i) = &
!!!          - (dalt_f(i)**2)/(4*dalt_f(i+1)*(dalt_f(i)+dalt_f(i+1))) * var(i+1) &
!!!          + (2*dalt_f(i+1)+dalt_f(i))/(4*dalt_f(i)) * var(i) &
!!!          + (2*dalt_f(i+1)+dalt_f(i))/(4*(dalt_f(i)+dalt_f(i+1))) * var(i-1)
!!!
!!!  end do
!!!
!!!
