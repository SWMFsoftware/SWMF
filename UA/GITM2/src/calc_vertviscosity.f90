...
subroutine calc_vertviscosity(Quantity, Diff, MulFac, dTdt_cond)

  use ModSizeGitm, only : nAlts
  use ModVertical, only: dAlt_F, Altitude_G, InvRadialDistance_C
  use ModGITM, only: Dt
  use ModConstants

  implicit none

  real, intent(in) :: Quantity( -1:nAlts+2)
  real, intent(in) :: Diff(0:nAlts+1)
  real, intent(in) :: MulFac(0:nAlts+1)
  real, intent(out) :: dTdt_cond(nAlts)

  real, dimension(0:nAlts+1) :: m, du, r, du12, du22, &
       dl, lou, dlou, di, r2

  real, dimension(-1:nAlts+2) :: RadialDistance
  ...
  do iAlt = -1, nAlts + 2
    RadialDistance(iAlt) = 1.0/InvRadialDistance_C(iAlt)
  enddo 
  tempold = Quantity(0:nAlts+1)

  r2 = RadialDistance(0:nAlts+1)**2
  di = diff(:)*r2

  m = Dt/(MulFac(0:nAlts+1)*r2)
  du = Altitude_G(1:nAlts+2) - &
       Altitude_G(0:nAlts+1)
  dl = Altitude_G(0:nAlts+1) - &
       Altitude_G(-1:nAlts+0)
  r = du/dl
  ...
  enddo
  dTdt_cond(1:nAlts) = temp(1:nAlts) - Quantity(1:nAlts)

  call end_timing("conduction")

end subroutine calc_vertviscosity
