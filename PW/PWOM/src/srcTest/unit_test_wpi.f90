program unit_test_wpi
  use ModParticle, ONLY: test_wpi
  use ModMPI
...
!  character(len=7) :: NamePlanet = 'JUPITER'
  ...
  write(*,*) 'starting test_wpi'

  call timing_active(.true.)
  call timing_step(0)
  call timing_start('unit_test_wpi')
  
  call timing_start('test_wpi')
  call test_wpi
  call timing_stop('test_wpi')

  call timing_report
end program unit_test_wpi
