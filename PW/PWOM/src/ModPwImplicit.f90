module ModPwImplicit
  ...
  subroutine PW_calc_residual(nOrder, DtIn, nCell, nVar, StateIn_GV, Resid_CV)
    ...
    !Update source terms
    call COLLIS(nCell,StateIn_GV)
    call PW_calc_efield(nCell,StateIn_GV)         
...
!    else if (nOrder == 2) then
       do iIon=1,nIon-1
          call rusanov_solver(iIon,RGAS_I(iIon),DtIn,   &
               StateIn_GV(-1:nCell+2,iRho_I(iIon):iT_I(iIon)),&
               Source_CV(1:nCell,iRho_I(iIon)), Source_CV(1:nCell,iU_I(iIon)),&
               Source_CV(1:nCell,iP_I(iIon)),  &
               NewState_GV(-1:nCell+2,iRho_I(iIon):iT_I(iIon)))
       enddo
!    endif
    ...
    Resid_CV(1:nCell,RhoE_)=0.0
    Resid_CV(1:nCell,uE_)  =0.0
    do k=1,nCell
       do iIon=1,nIon-1
          Resid_CV(K,RhoE_) = &
  ...
  subroutine PW_update_boundary(nCell, nVar, State_GV)
    ...
    !---------------------------------------------------------------------------
    do iIon = 1,nIon-1
       State_GV(nCell+1:nCell+2,iU_I(iIon)) = State_GV(nCell,iU_I(iIon))
       State_GV(nCell+1:nCell+2,iT_I(iIon)) = State_GV(nCell,iT_I(iIon))

       ! cBoltzmann [cgs] = 1.0e7 * cBoltzmann [SI] 
       ScaleHeight_I(iIon) =&
            1.0e7*cBoltzmann*(State_GV(nCell,iT_I(iIon))+State_GV(nCell,Te_))&
            /(abs(Gravty(nCell))*Mass_I(iIon))

       State_GV(nCell+1:nCell+2,iP_I(iIon)) = &
            State_GV(nCell,iP_I(iIon))*exp(-DrBnd/ScaleHeight_I(iIon))

       State_GV(nCell+1,iRho_I(iIon)) = &
            State_GV(nCell+1,iP_I(iIon)) / RGAS_I(iIon) / State_GV(nCell+1,iT_I(iIon))

       State_GV(nCell+2,iRho_I(iIon)) = &
            State_GV(nCell+2,iP_I(iIon)) / RGAS_I(iIon) / State_GV(nCell+2,iT_I(iIon))
    enddo

    XHTM=1.+ELFXIN*EXP(-(TIME-300.)**2/2./150./150.)

    ! Top boundary electron temperature is calculated from
    ! Te(nCell) + Tgradient*Dx where Tgradient = Etop/kappa
    ! and Etop is the electron heat flux or proportional to that
    State_GV(nCell+1,Te_)=State_GV(nCell,Te_)+XHTM*ETOP/State_GV(nCell,Te_)**2.5
    ...
    if (UseGmToPw) then
       State_GV(nCell+1,iP_I(Ion1_)) = &
            (  State_GV(nCell+1,iT_I(Ion1_))&
            / (State_GV(nCell+1,iT_I(Ion1_))+State_GV(nCell+1,Te_))&
            )*p_I(iLine)
       State_GV(nCell+1,iRho_I(Ion1_)) = &
            State_GV(nCell+1,iP_I(Ion1_)) / RGAS_I(Ion1_) / State_GV(nCell+1,iT_I(Ion1_))
    endif
    ...
    State_GV(nCell+1,RhoE_)=0.0
    State_GV(nCell+1,uE_)  =0.0

    do iIon=1,nIon-1
       State_GV(nCell+1,RhoE_) = &
            State_GV(nCell+1,RhoE_)+MassElecIon_I(iIon)*State_GV(nCell+1,iRho_I(iIon))
       State_GV(nCell+1,uE_)= &
            State_GV(nCell+1,uE_)+ &
            (MassElecIon_I(iIon)*State_GV(nCell+1,iRho_I(iIon))&
            *State_GV(nCell+1,iU_I(iIon)))
    enddo
    State_GV(nCell+1,uE_)=&
         (State_GV(nCell+1,uE_) -1.8965E-18*CURRMX)/State_GV(nCell+1,RhoE_)

    State_GV(nCell+1,pE_)=&
         RGAS_I(nIon)*State_GV(nCell+1,Te_)*State_GV(nCell+1,RhoE_)  
