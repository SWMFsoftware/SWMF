...
!!!!!!!!!!!!!!!!!!!!!!!!
        ...
        !comment out when crashes from here (Mike)
...
subroutine GEOSB
  ...
  if (I7.eq.1 .or. IG7.eq.1) then	! LANL data injection
     ...
     do S=2,NS
        NMFAC(S)=NY(S)/FAC
        ! The next line is for the idealized test runs only, no BC variation:
        !  NMFAC(S)=1.
     end do
     ...
     do S=2,NS
        FSFAC(S)=NY(S)/FAC
        ! The next line sometimes gives bad results...use with care.
