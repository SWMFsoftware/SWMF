    ...
    do iRadialPoint = 1, nRadialPoint
       do iStep =1, nStep
          do iPoint = 1, nPoint
             ...
             LnHDensity(iRadialPoint,iStep) = LnRairdenDensity(iR)+(RadialDistance(iRadialPoint,iStep)&
                  -RairdenDistance(iR))*(LnRairdenDensity(iR+1)-LnRairdenDensity(iR))&
                  /(RairdenDistance(iR+1)-RairdenDistance(iR))
             EXIT
          end do
...
!=================================================================================
  ...
  subroutine test_density
    ...
    do iRadialPoint = 1, nRadialPoint
       ...
       call find_mirror_points (nStep, nPitch, PitchAngle_I, bField_I(iRadialPoint,:), &
            bMirror_I(iRadialPoint,:),iMirror_II(iRadialPoint,:,:))
       
       bMirror(iRadialPoint) = bMirror_I(iRadialPoint,1)
       
       call get_rairden_density(nStep, iRadialPoint,L(iRadialPoint), HDensity(iRadialPoint,:))
       
       call get_hydrogen_density(nStep, iMirror_II(iRadialPoint,:,:), bMirror(iRadialPoint),&
            bField_I(iRadialPoint,:), dLength_I, L(iRadialPoint), AvgHDensity(iRadialPoint), HDensity(iRadialPoint,:))
