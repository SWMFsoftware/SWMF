...
contains
  ...
  subroutine get_rairden_density(nStep, nRadialPoint,L_I, HDensity_II)
    ...
    !iUnitRairden = io_unit_new(); use unittmp_ 
    iUnitRairden = 11
    ...
    do iRadialPoint = 1, nRadialPoint
       do iStep =1, nStep
          do i = 1, n
             weight_I(i) = (Rad_I(i)-RadialDistance_II(iRadialPoint,iStep))/dR
          ...
          HDensity_II(iRadialPoint,iStep) = weight_I(iR+1)*H_I(iR)+weight_I(iR)*H_I(iR+1)
...
!!$  end subroutine test_density
  ...
  subroutine test_density
    ...
    write(*,*) 'HDensity_II(iRadialPoint,iStep)',HDensity_II(nRadialPoint,:) 
