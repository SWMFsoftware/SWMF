...
subroutine GM_SetGrid(MLTsIn, LatsIn, iError)
  ...
  do i=1,GMi_NeednLats
     do j=1,GMi_NeednMLTs
        ...
        call IE_FindPoint(GMr1_Location, IEr1_Location, iError)

        if (iError == 0) then
           GMi3_InterpolationIndices(j,i,1:3) = IEr1_Location(1:3)
           GMr3_InterpolationRatios(j,i,1:2)  = IEr1_Location(4:5)
        else
...
subroutine GM_GetPotential(PotentialOut, iError)
  ...
  integer, intent(out) :: iError
  real, dimension(IEi_HavenMlts,IEi_HavenLats,IEi_HavenBLKs) :: IE_Value
  ...
  call GM_GetValue(IE_Value, ValueOut, iError)
...
subroutine GM_GetValue(ValueIn, ValueOut, iError)
  ...
  integer, intent(out) :: iError
  real, dimension(IEi_HavenMlts,IEi_HavenLats,IEi_HavenBLKs), intent(in) :: &
       ValueIn
