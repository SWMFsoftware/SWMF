...
!        WRITE(22,7) T/3600
	OPEN(UNIT=22,FILE='wmdfc_16'//ST2//'.in',STATUS='OLD')
	READ(22,20) HEADER
        OPEN(UNIT=23,FILE='wmdfc_18'//ST2//'.in',STATUS='OLD')
        READ(23,20) HEADER
        OPEN(UNIT=24,FILE='wmdfc_20'//ST2//'.in',STATUS='OLD')
        READ(24,20) HEADER
...
!         write(22,17)kn,enor(kn)
          read(22,17)kk,enor(kn,1) 
	  read(23,17)kk,enor(kn,2)
	  read(24,17)kk,enor(kn,3)
!	 CALL WPIDC(ENOR(KN),S,0.15,0.1,0.2,500.,4.)
!         DO L=1,NPA-1
!          NDAAJ(KN,L)=NDAA3(L)          ! normalized diff coeff Daa
!         ENDDO
!         write(22,27)(ndaaj(kn,l),l=1,LO)
	  read(22,27) (ndaaj(kn,l,1),l=1,LO)
	  read(23,27) (ndaaj(kn,l,2),l=1,LO)
	  read(24,27) (ndaaj(kn,l,3),l=1,LO)
100     CONTINUE
...
!	READ(10,20) HEADER
	OPEN(UNIT=11,FILE=ST4//'_o.wans',STATUS='OLD')
	READ(11,20) HEADER
	OPEN(UNIT=9,FILE=ST1//ST2//'.wgr',STATUS='UNKNOWN')
	WRITE(9,55) T/3600,KP
55	FORMAT(2HT=,F6.1,2X,3HKp=,F3.1,X,   &
      	21HL  PHI  GAIN[dB]  XFR)
	OPEN(UNIT=8,FILE=ST1//ST2//'.wans',STATUS='UNKNOWN')
	WRITE(8,56) T/3600,KP     
56	FORMAT(2HT=,F6.1,2X,3HKp=,F3.1,X,   &
      	'L   PHI     ANIS   EDEN[keV/cm3]   RNHT[1/cm3]',3X,   &
      	'PPER[keV/cm3] PPAR[keV/cm3]')
	OPEN(UNIT=22,FILE=ST1//ST2//'.wdc',STATUS='UNKNOWN')
	WRITE(22,7) T/3600,KP     
...
!	  READ(10,551) RLD,PHID,AII(1,j),EPO(1,j),DN(1,j)  ! moder He+
	  READ(11,551) RLD,PHID,AII(2,j),eden1,DN(2,j),pero,paro
...
!	WRITE(8,551)LZ(I),PHI(J),ANIS(I,J,S),EPAR(I,J,S),RNHT(I,J,S)
	WRITE(8,551)LZ(I),PHI(J),ANIS(I,J,S),EDEN(I,J,S),RNHT(I,J,S),   &
...
!          delw=wm/4.
!          delf=delw*cubL/(3.75)**3
	IF(S.EQ.2) CALL WGRDAR(LZ(I),XNE(I,J),RNHT(I,J,S),EPAR(I,J,S),ANIS(I,J,S),   &
...
!	   BWAVE(I,J)=EXP((2*WCD(I,J)-40.)/8.6859)
	  ...
	  ENDIF
	  WRITE(9,10)LZ(I),PHI(J),WCD(I,J),xfr(i,j),BWAVE(I,J)
...
!.....zero out bounce-averaged quantities and bounce-average
	...
	if (lz(i).eq.(4).and.k.eq.kprint)then
	 write(22,550)
	 write(22,*)'L=',LZ(I),' MLT=',MLT(J),' Y=',xfr(i,j),   &
...
!.....write out bounce-averaged results	
	 ...
	 if(taudaa.lt.1e-20) taudaa=1e-20
         write(22,552) ekev(k),PAbn(L),taudaa,1/taudaa
	endif
...
!.......Calculation of perpendicular current [nA/m2]
	OPEN(UNIT=3,FILE=ST1//ST2//'.jper',STATUS='UNKNOWN')
	WRITE(3,156) T/3600,KP     
