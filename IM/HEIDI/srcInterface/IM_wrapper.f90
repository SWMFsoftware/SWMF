...
contains
  ...
  subroutine IM_put_from_gm_line(nRadiusIn, nLonIn, Map_DSII, &
    ...
    do iT = 1,nT-1
       do iR = 1, nR
          if (LZ(iR) > rBoundary) then
             ...
             call fill_dipole_north( &
                  nStepInside+1, LZ(iR), Phi(iT), LatBoundaryN, &
                  XyzDipoleN_VI, bDipoleN_VI,&
                  bDipoleMagnN_I, sDipoleN_I, rDipoleN_I)
             call fill_dipole_south( &
                  nStepInside+1, LZ(iR), Phi(iT), LatBoundaryS, &
                  XyzDipoleS_VI, bDipoleS_VI,&
                  bDipoleMagnS_I, sDipoleS_I, rDipoleS_I)

             Xyz_VIII(:,1:nStepInside,iR,iT)                 = &
                  XyzDipoleS_VI(:,1:nStepInside)
             Xyz_VIII(:,(nStep-nStepInside +1):nStep,iR,iT)  = &
                  XyzDipoleN_VI(:,2:nStepInside+1)

             BHeidi_III(1:nStepInside,iR,iT)                 = &
                  bDipoleMagnS_I(1:nStepInside)
             BHeidi_III((nStep-nStepInside +1):nStep,iR,iT)  = &
                  bDipoleMagnN_I(2:nStepInside+1)

             RHeidi_III(1:nStepInside,iR,iT)                 = &
                  rDipoleS_I(1:nStepInside)   
             RHeidi_III((nStep-nStepInside +1):nStep,iR,iT)  = &
                  rDipoleN_I(2:nStepInside+1)

             SHeidi_III(1:nStepInside,iR,iT)                 = &
                  sDipoleS_I(1:nStepInside)   
             ...
             do i = iPointEq, iPointEq
                SHeidi_III(i,iR,iT) = &
                     sDipoleS_I(nStepInside) + &
                     STemp(iPointEq-1,iR,iT) + STemp(iPointEq,iR,iT)
             end do
             ...
             do i = nStep-nStepInside+1 , nStep
                ...
                SHeidi_III(i,iR,iT) = &
                     SHeidi_III(nStep-nStepInside,iR,iT) + sDipoleN_I(j)
             end do

             !\
             ! Fill in dipole values for mhd field lines beyond rBoundary: 
             ! LatBoundary = atan(z/sqrt(x^2+y^2))
             !/

             BxHeidi_III(1:nStepInside,iR,iT)                = &
                  bDipoleS_VI(1,1:nStepInside) 
             BxHeidi_III((nStep-nStepInside +1):nStep,iR,iT) = &
                  bDipoleN_VI(1,2:nStepInside+1)

             ByHeidi_III(1:nStepInside,iR,iT)                = &
                  bDipoleS_VI(2,1:nStepInside) 
             ByHeidi_III((nStep-nStepInside +1):nStep,iR,iT) = &
                  bDipoleN_VI(2,2:nStepInside+1)

             BzHeidi_III(1:nStepInside,iR,iT)                = &
                  bDipoleS_VI(3,1:nStepInside) 
             BzHeidi_III((nStep-nStepInside +1):nStep,iR,iT) = &
                  bDipoleN_VI(3,2:nStepInside+1)

             pHeidi_III(1:nStepInside,iR,iT)                = &
                  pHeidi_III(nStepInside+1,iR,iT)  
             pHeidi_III((nStep-nStepInside +1):nStep,iR,iT) = &
                  pHeidi_III(nStep-nStepInside,iR,iT)

             RhoHeidi_III(1:nStepInside,iR,iT)                = &
                  RhoHeidi_III(nStepInside+1,iR,iT)  
             RhoHeidi_III((nStep-nStepInside +1):nStep,iR,iT) = &
                  RhoHeidi_III(nStep-nStepInside,iR,iT)


          end if

          if (LZ(iR) <= rBoundary) then
             call fill_dipole(nStep, LZ(iR), Phi(iT), XyzDipole_VI, &
                  bDipole_VI, bDipoleMagn_I, sDipole_I, rDipole_I)
    ...
    do iPhi =1, 1
       do iR =6, 8,2
          do iPoint =1, nStep
             write(9,*) iPoint, iR, BHeidi_III(iPoint,iR,iPhi),&
                  BxHeidi_III(iPoint,iR,iPhi), ByHeidi_III(iPoint,iR,iPhi),&
                  BzHeidi_III(iPoint,iR,iPhi), &
                  Xyz_VIII(1,iPoint,iR,iPhi),  &
                  Xyz_VIII(2,iPoint,iR,iPhi),  &
                  Xyz_VIII(3,iPoint,iR,iPhi),  &
                  SHeidi_III(iPoint,iR,iPhi),  &
                  atan(Xyz_VIII(3,iPoint,iR,iPhi) &
                  /(sqrt(Xyz_VIII(1,iPoint,iR,iPhi)**2 &
                  +      Xyz_VIII(2,iPoint,iR,iPhi)**2)))
          end do
       end do
    end do
    close(9)
  ...
  subroutine IM_run(SimTime, SimTimeLimit)
    ...
    real, intent(in)    :: SimTimeLimit ! simulation time limit
...
!!! call heidi_save_restart
  ...
  subroutine IM_put_from_gm_crcm(Integral_IIV,iSizeIn,jSizeIn,nIntegralIn,&
    ...
    call CON_stop(NameSub//': IM_ERROR: should not be called for IM/HEIDI!')
  ...
  subroutine interpolate_linear_b( &
    ...
    integer, intent(in)  :: nP ! number of points along BATSRUS field line
  ...
  subroutine interpolate_mhd(nOrder, nStepMhd, nStep, &
       lMhd_I, bMhd_I, bHeidi_I, lHeidi_I)
  ...
  subroutine fill_dipole(nStep, L, Phi, XyzDipole_VI, &
       bDipole_VI, bDipoleMagn_I, sDipole_I, rDipole_I)
  ...
  !============================================================================
  subroutine fill_dipole_south(nStep, L, Phi, LatBoundary, &
       XyzDipoleS_VI, bDipoleS_VI, bDipoleMagnS_I, sDipoleS_I, rDipoleS_I)
  ...
  !============================================================================
  subroutine fill_dipole_north(nStep, L, Phi, LatBoundary, &
       XyzDipoleN_VI, bDipoleN_VI, bDipoleMagnN_I, sDipoleN_I, rDipoleN_I)
