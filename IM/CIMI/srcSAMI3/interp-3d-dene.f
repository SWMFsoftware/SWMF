...
!      open interpolated data files
       ...
       open(unit=15,file='dene0i.dat', form='unformatted')
     ...
     .            k,glon0(nx/2,ny,k)
       ...
       do n = 1,nt
         do k = 2,nl-1
           do j = 2,ny-1
             do i = 2,nx-1
!               if ( dene0(i,j,k,n) .eq. 1. ) 
!     .            dene0(i,j,k,n) = !dene0i(i,j,k-1,n)
                  dene0(i,j,k,n) = 
     .                           ( dene0i(i-1,j-1,k-1,n) +
     .                             dene0i(i+1,j-1,k-1,n) +
     .                             dene0i(i-1,j+1,k-1,n) +
     .                             dene0i(i+1,j+1,k-1,n) +
     .                             dene0i(i-1,j-1,k+1,n) +
