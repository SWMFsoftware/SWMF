...
$$
about the intregrals like this see \S58 in \cite{ll}. Therefore the total energu density in the Planck spectrum equals $\alpha T^4$, as it should.

Comment about the case in which Emitter or Absorber is an electron, obeying the Fermi statistics.
...
\end{equation}
Of course, if the radiation spectrum differs from that for equilibrium black-body radiation, the values of $T^{Spectral}$ are different at different energies, 
generally speaking.
...
follows:
\begin{equation}\label{eq:mg14}
T^{Spectral}=\frac{
\varepsilon/k_B}
{
\log\left(1+\frac{8\pi}{h^3c^3}\frac{\varepsilon^3}{E_\varepsilon}\right)
}
\end{equation} 
...
straightforward. However, the inverse procedure is not that simple, because Eq.(\ref{eq:mg14}) now only holds as an approximate one:
...
\end{equation}
To satisfy Eq.(\ref{eq:mg15}), one needs to improve the accuracy using Newton-Rapson procedure:
\begin{equation}
