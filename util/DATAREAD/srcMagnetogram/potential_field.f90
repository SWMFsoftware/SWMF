...
contains
  ...
  subroutine read_magnetogram
    ...
    ! Name of input file
    character (len=*), parameter:: NameFileIn = 'test.in'

    real, parameter:: BrMax = 1900.0
  ...
  subroutine init_potential_field
    ...
    allocate( &
         ...
         SinTheta_I(nTheta), SinThetaNode_I(nTheta+1), &
    ...
    SinTheta_I = sin(Theta_I(1:nTheta))
...
contains
  ...
  subroutine get_gradient(x_C, Grad_DG)
    ...
    use ModPotentialField, ONLY: nR, nTheta, nPhi, Radius_I, SinTheta_I, &
         dRadiusNode_I, dThetaNode_I, dPhiNode_I, Br_II
...
program potential_field
  ...
  use ModB0Matvec, ONLY: get_gradient, get_divergence, matvec, UseBr
  use ModLinearSolver, ONLY: gmres, bicgstab
  use ModPlotFile, ONLY: save_plot_file

  implicit none

  integer :: nKrylov=400, nIter=1000
