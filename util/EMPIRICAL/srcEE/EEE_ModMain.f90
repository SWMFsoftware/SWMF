...
module EEE_ModMain
  ...
  public EEE_initialize, get_transformed_TD99fluxrope
...
contains
  ...
  subroutine EEE_initialize(BodyNDim,BodyTDim,gamma_in)

    real, intent(in) :: BodyNDim,BodyTDim,gamma_in
    ...
    No2Si_V(UnitX_)   = rSun
    No2Si_V(UnitU_)   = sqrt(gamma*cBoltzmann*BodyTDim/cProtonMass)
  ...
  subroutine init_TD99_parameters
    ...
    !/
    !--------------------------------------------------------------------
    call MPI_COMM_RANK(MPI_COMM_WORLD,iProc,errcode)
    ...
    if (iProc==0) then
        ...
        write(*,*) prefix,'>>>>>>>>>>>>>>>>>>>                   <<<<<<<<<<<<<<<<<<<<<'
        write(*,*) prefix
        write(*,*) prefix,'>>>>     Normalization Units (in MKS) in the model.    <<<<'
        write(*,*) prefix
        write(*,*) prefix,'X0 = ',No2Si_V(UnitX_),'[m] = ',No2Si_V(UnitX_)/1.0e6,'[Mm] = Rsun'
        write(*,*) prefix,'B0 = ',No2Si_V(UnitB_),'[T] = ',No2Si_V(UnitB_)*1.0e4,'[Gauss]'
        write(*,*) prefix,'I0 = ',No2Si_V(UnitJ_),'[A] = ',No2Si_V(UnitJ_)*1.0e6,'[microA]'
        write(*,*) prefix
        write(*,*) prefix,'>>>>       Normalized values of model parameters.      <<<<'
        write(*,*) prefix
        write(*,*) prefix,'d_TD99      = ',d_TD99,'[X0] ',              &
            d_TD99*No2Si_V(UnitX_)/1.0E6,'[Mm]'
        write(*,*) prefix,'Rtube_TD99  = ',Rtube_TD99,'[X0] ',          &
            Rtube_TD99*No2Si_V(UnitX_)/1.0E6,'[Mm]'
        write(*,*) prefix,'atube_TD99  = ',atube_TD99,'[X0] ',          &
            atube_TD99*No2Si_V(UnitX_)/1.0E6,'[Mm]'
        write(*,*) prefix,'atube/Rtube = ',atube_TD99/Rtube_TD99,'[-]'
        write(*,*) prefix,'Itube_TD99  = ',Itube_TD99,'[I0] ',          &
            ItubeDim,'[A]'
        write(*,*) prefix,'aratio_TD99 = ',aratio_TD99,'[-]'
        write(*,*) prefix,'Mass_TD99   = ',Mass_TD99*1.0e3,'[g] ',        &
            'InvH0_TD99  = ',InvH0_TD99,'[1/X0]' 
        write(*,*) prefix,'Rho0_TD99   = ',Rho0_TD99,'[Rho0] = ',       &
            Rho0_TD99*No2Io_V(UnitRho_),'[g/cm^3]'
        write(*,*) prefix
        write(*,*) prefix,'q_TD99      = ',q_TD99,'[B0*X0^2] ',         &
            q_TD99*No2Si_V(UnitB_)*No2Si_V(UnitX_)**2,'[T m^2]'
        write(*,*) prefix,'L_TD99      = ',L_TD99,'[X0] ',              &
            L_TD99*No2Si_V(UnitX_)/1.0e6,'[Mm]'
        write(*,*) prefix
        write(*,*) prefix,'Free energy of flux rope is ',WFRope,'Ergs.'
        write(*,*) prefix,'Separation of flux rope ends is ',FootSepar, &
            'Mm, or',cPi*FootSepar*1.0e6/(2.0*Rsun)*cRadToDeg,'deg.'
        write(*,*) prefix
