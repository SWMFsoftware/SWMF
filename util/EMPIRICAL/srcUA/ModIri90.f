!       Fri Jan  4 12:28:19 EST 2008  <-RegFa1 with interface
...
!       Wednesday, January 02, 2008
       ...
       end module EUA_BLOTE

       module       EUA_BLOTN
           real  :: XSM1,TEXOS,TLBDH,SIGMA 
       end module   EUA_BLOTN
       ...
       module       EUA_BLOCK8
          ...
          integer              :: MXSM    
       end module   EUA_BLOCK8    

       module       EUA_BLOCK7
          real   :: D1,XKK,FP30,FP3U,FP1,FP2
       end module   EUA_BLOCK7

       module       EUA_BLOCK6
          real   :: HMD,NMD,HDX   
       end module   EUA_BLOCK6
                               !v10 it is something wrong possible with NMD
       module       EUA_BLOCK5
        logical  :: NIGHT
          real   :: E(4)
       end module   EUA_BLOCK5   

       module       EUA_BLOCK4
         real    :: HME,NME,HEF
       end module   EUA_BLOCK4   

       module       EUA_BLOCK3
         real    :: HZ,T,HST,STR
       end module   EUA_BLOCK3

       module        EUA_BLOCK2
           real   :: B0,B1,C1    
       end module    EUA_BLOCK2

       module        EUA_BLOCK1
           real   :: HMF2,XNMF2,HMF1       
       end module    EUA_BLoCK1

       module        EUA_BLO10
           real   :: BETA,ETA,DELTA,ZETA
       end module    EUA_BLo10       
...
C^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
     ...
     &                  ZKM,NZ,CCIRNM,URSINM,OUTF,OARR,iProc)
       ...
       use EUA_BLOTE    !/AHH,ATE1,STTE,DTE
       use EUA_BLOTN    !/XSM1,TEXOS,TLBDH,SIGMA /EUA_BLOTE/AHH,ATE1,STTE,DTE
       use EUA_BLOCK8   !/HS,TNHS,XSM,MM,DTI,MXSM    
       use EUA_BLOCK7   !/D1,XKK,FP30,FP3U,FP1,FP2
       use EUA_BLOCK6   !/HMD,NMD,HDX   
       use EUA_BLOCK5   !/NIGHT,E
       use EUA_BLOCK4   !/HME,NME,HEF  
       use EUA_BLOCK3   !/HZ,T,HST,STR
       use EUA_BLoCK2
       use EUA_BLock1,    NMF2=>xnmf2

cv10         /EUA_BLOCK1/HMF2, NMF2,HMF1
cv10     real  ::   HMF2,XNMF2,HMF1
       use EUA_BLo10
...
C          Declarations for Apr 94 changes
       ...
       parameter ( UMR= 1.7453292519943295D-02 )
       double precision        ARGMAX
       parameter (        ARGMAX=87.0            )
...
cv10      real UMR !=    1.7453292519943295D-02  !        /EUA_CONST/UMR



                  !      &  /EUA_BLOCK4/ HME,NME,HEF  
                  !      &  /EUA_BLOCK5/ logical::NIGHT, real,dim(4)::E
                  !      &  /EUA_BLOCK6/ HMD,NMD,HDX   
                  !      &  /EUA_BLOCK7/ D1,XKK,FP30,FP3U,FP1,FP2
                  !      &  /EUA_BLOCK8/ HS,TNHS,XSM,MM,DTI,MXSM    
                  !          &  /EUA_BLOTN / XSM1,TEXOS,TLBDH,SIGMA 
                  !         /EUA_BLOTE/AHH,ATE1,STTE,DTE
                  !      &  /EUA_BLOCK3/ HZ,T,HST,STR
                  !         /EUA_BLOCK1/ HMF2, NMF2,HMF1
                  !         real  ::    HMF2,XNMF2,HMF1       
...
Cv10     UMR=ATAN(1.0)*4./180.
cv10        UMR=    1.7453292519943295D-02
...
C Thus avoiding DATA statement with parameters from COM-MON block.
C
        AHH(1)=120.
        AHH(2)=0.
        AHH(3)=300.
      ...
      TENEOP=(.NOT.JF(10))
        IF(TENEOP) THEN
           ...
           DO     JXNAR  = 1,3   !8154
             ...
             TCON(JXNAR) =.FALSE. 
8154         IF(XNAR(JXNAR) > 0.) TCON(JXNAR)=.TRUE. 
           end do
         ENDIF
...
C !!!!!!!!!! TE-MAXIMUM (JICAMARCA,ARECIBO) !!!!!!!!!!!!!!!!!!!!
      ...
      TMAXD =800.*EXP(-(MLAT/33.)**2)   +1500.0d0

      TMAXN =TN(HMAXN,TEXNI,TLBDN,SIGNI)+20.0d0
...
C !!!!!!!!!! ALSO TE > TN ENFORCED !!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   ...
   !      TNAHH2=TN(AHH(2),TEXOS,TLBDH,SIGMA)
      TNAHH2=TN(xxxAH  ,TEXOS,TLBDH,SIGMA)
   ...
   !   TNAHHI =TN(AHH(I+1),TEXOS,TLBDH,SIGMA)
       TNAHHI =TN(xxxAH,TEXOS,TLBDH,SIGMA)   

        IF(ATE(I+1) .LT. TNAHHI)  then
           ATE(I+1)=TNAHHI
        end if
...
C
      ...
      CALL SUFE(PG1O,RIF,12,PF1O, 80)
      CALL SUFE(PG2O,RIF, 4,PF2O, 32)
      CALL SUFE(PG3O,RIF,12,PF3O, 80 )
c
...
300   IF(NODEN) GOTO 330
      ...
      IF((HEIGHT.GT.HNEE).OR.(HEIGHT.LT.HNEA)) GOTO 330
       ...
       IF(LAYVER) THEN
         ELEDE=-9.
         IF(IIQU.LT.2) then
            ELEDE=XEN(HEIGHT,HMF2,NMF2,HME,4,HXL,SCL,AMP)
         end if
       ELSE
         ELEDE=XE(HEIGHT)
       ENDIF
...
7108  IF(NOION) GOTO 7118
      IF((HEIGHT.GT.HNIE).OR.(HEIGHT.LT.HNIA)) GOTO 7118
        ...
        if(DY) then
           xxhh =xhi*umr
           xxll =lati*umr
cv10      call IONCOM(HEIGHT, XHI*UMR, LATI*UMR, COV, MONTH,DION)
          call IONCOM(HEIGHT, xxhh,     xxll,  COV,  MONTH,DION)
...
C-------------------------------------------------------------------
       ...
       integer, intent(in out)::  month,  iday, idoy
       integer :: imo, mobe, mo, mooLd
...
C
      real FUNCTION HPOL(HOUR,TW,XNW,SA,SU,DSA,DSU)            
       implicit NONE
C-------------------------------------------------------
...
C BILITZA----------------------------------------- 1979.
        real, intent(in):: Hour,tw,xnw, sa, su,dsa,dsu
        real ::xx,yy
        ...
        IF(ABS(SU).GT.25.) THEN
              ...
              RETURN
        ENDIF
        xx  = (XNW-TW)*EPST(HOUR,DSU,SU) 
        yy  = (TW-XNW)*EPST(HOUR,DSA,SA)
        HPOL= XNW +yy +xx
 
c      HPOL=XNW+(TW-XNW)*EPST(HOUR,DSA,SA)+
c     &         (XNW-TW)*EPST(HOUR,DSU,SU) 
...
C DEN IS THE ELECTRON DENSITY IN M-3.  
       ...
       real  :: xx , yy     
        ...
        xx = (-5.122E-4*H+(6.094E-12-3.353E-14*H)*DEN) 
        yy =  1051.+(17.01*H-2746.)
      ...
      IF(COV.LT.0.)   then
        xx = -(ACOV  -115.0d0)/10.d0
        xx = 1.d0    + EXP(xx)
        yy = 0.123d0 + 1.69d-3*ACOV
      else
        xx = -(ACOV  -102.5d0)/5.d0
        xx = 1.0d0   +EXP(xx)  
        yy = 0.117d0 +2.02d-3*ACOV
      end if
...
C---------------------------------------------------------------  
       ...
       real, intent( in)    :: xnmf2, xnmf1, xnme, vne, hmf2
       real, intent(in)     :: hmf1, hme, hv1, hv2, hhaLf
       real, intent(out)    :: hxL
       ...
       LOGICAL,intent(in)  ::    NIGHT
          Logical :: ssin  
          integer :: numcon, nc0
          real    :: xx,yy, ww, amp,  scL, aLogf, aLogef, xhalf
          real    :: scL0 , zet, hfff, xfff, hxL1t
c
c constants --------------------------------------------------------
          integer NUMLAY; parameter(    NUMLAY = 4)
          integer NC1   ; parameter(    NC1    = 2)
          real ALG102   ;
               ALG102 =ALOG10(2.) 
c
...
C
       ...
       real, intent(in)    :: hour, sax, sux, r, dela
       integer, intent(in) :: iseason

       real    :: b0f, siph, sipL, dayvaL
       integer :: isr, isL
...
c      REAL, intent(in)   :: MLONG !, MLAT,LONG
      real, intent(in out)  :: mLong,Lati,Long,mLat
cv10      /EUA_CONST/FAKTOR
       double precision   FAKTOR 
       parameter (        FAKTOR = 1.7453292519943295D-02 )

       real :: zpi,cbg, ci, si,cbm, sbm
       real :: cLm, sLm, sbg, yLg, cLg,sLg
...
C SHEIK,1977.         
      ...
      real, intent(out)   :: dip, dec,smodip

      
      integer :: k, i, nmax, ih,iL, ihmax, Last, imax, m
      real    :: feL1, feL2,g, rLat,rLong,  ct, st, d,cp,sp,h,s
      real    :: zzz, rho, xxx, yyy, rq, xi, f1, x1, y1, z1,xt
      real    :: brh0, zdivf,  ydivs, dipdiv
...
10     G(72+K)=FEL2(I)         
      ...
      D=SQRT(40680925.0d0 -272336.0d0*CT*CT)   
      ...
      XXX=RHO*CP      
      YYY=RHO*SP      
      RQ=1.0d0/(XXX*XXX +YYY*YYY +ZZZ*ZZZ)    
      XI(1) =XXX*RQ    
      XI(2) =YYY*RQ    
      XI(3) =ZZZ*RQ    
      IHMAX =NMAX*NMAX+1          
...
C
      SUBROUTINE SUFE (FIELD,RFE,M,FE, maxK)             
...
C CHOSEN HEADER NUMBERS.FE(M) IS THE CORRESPONDING SET.        
      implicit none
      integer, intent(in) :: m, maxK
      real                :: rfe,   fe,   field   , efe
      DIMENSION RFE(4),FE(*),FIELD(*),EFE(4)
      
      integer :: k, i, kafka

      kafka = maxK !!! size(field, 1)    
...
111    FE(I)=FIELD(K) 
      ...
      DO  I=1,4    !120
       IF((EFE(I) .GT. -10.0) .AND. ( RFE(I).NE.EFE(I))) then
        if( (k+4+m) <= kafka ) then 
                 GOTO 100     
        else
                 write(*,*)"Line #1444:  wrong K "
        end if
       end if
!v10 ^^^^^^^^^^^^ it was possibe unlimited K, outside of FIELD[] bounds 
120   CONTINUE        
      end do
...
c------------------------------------------------------------------
      ...
      real    :: sum,aa,bb, xxsi, xi,sm
      integer :: i        


cv10        /EUA_ARGEXP/       ARGMAX
       double precision        ARGMAX
       parameter (        ARGMAX=87.0            )
      ...
      DO   I=1,M   ! 100
             XI  =ID(I)
             xxsi=xs(i)
!v10          aa = eptr(h ,xi,xs(i))
              aa = eptr(h ,xi,xxsi)

!v10          bb = eptr(h0,xi,xs(i))
              bb = eptr(h0,xi,xxsi)
...
c----------------------------------------------------------------
      use     EUA_BLOCK8, G=>DTI, M=>MXSM  !/HS,TNHS,XSM, MM, DTI, MXSM 
      implicit none

!      REAL               MM
!      /EUA_BLOCK8/        HS,TNHS,XSM(4),MM(5),G(4),M

      real, intent(in):: h
      real   :: sum,aa,bb, xxgi, xxsmi
      integer:: i  
      ...
      DO  I=1,M-1  !100
        xxgi =g(i)
        xxsmi=xsm(i)
cv10       aa = eptr(h ,g(i),xsm(i))
           aa = eptr(h ,xxgi,xxsmi)

cv10       bb = eptr(hs,g(i),xsm(i))
           bb = eptr(hs,xxgi,xxsmi)

cv10 100        SUM=SUM+(MM(I+1)-MM(I))*(AA-BB)*G(I)                
100        SUM=SUM+(MM(I+1)-MM(I))*(AA-BB)*xxGI                
      end do
...
C EXP-FUNCTION.
c----------------------------------------------------------------
      use         EUA_BLOTE, ah=>ahh, st=>stte, d=>dte 
                          !/AHH,  ATE1, STTE ,DTE
cv10             /EUA_BLOTE/AH(7),ATE1, ST(6),D(5)
      implicit none
       real, intent(in):: h
       real            :: sum, aa, bb, xxdi, xxahi, xxah1
       integer         :: i
C
      ...
      DO  I=1,5    !1
          ...
          xxahi=ah(i+1)
          xxah1=ah(1)
cv10      aa = eptr(h    ,d(i),ah(i+1))
          aa = eptr(h    ,xxdi,xxahi)

cv10      bb = eptr(ah(1),d(i),ah(i+1))       
          bb = eptr(xxah1,xxdi,xxahi)

cv101         SUM=SUM+(ST(I+1)-ST(I))*(AA-BB)*D(I)     
1         SUM=SUM+(ST(I+1)-ST(I))*(AA-BB)*xxDI                
...
cCCC
        integer, intent(in) :: t
        real, intent(in)    :: h,z,f,fs
        real, intent(out)   :: cn
       
        integer :: i,j
        real    :: po, p, ph, pn, phe, pno, po2, pcL, s,  hm
        real    :: cm, aLL, var, hx, betL, xpon, aLh, beth
...
C     
C     **********************************************************************
      real :: xL,  gLat,   c,    dgtr, s, c2, c4, s2, pLg, tLL
      real :: stL, stLoc,  ctLoc,s2tLoc, c2tLoc, s3tLoc,c3tLoc
      real :: dfa, f107a , t1,   f1
      real :: f3,  t2,     cd14, dr,  cd18 ,hr, dr2, cd32, cd39
      real :: t3,  t5,     t7,  t71, t72
      real :: f2,          t8,  t81, t82, t14 
      real :: t11,   t12,  t4,  tLb,  g0
      real :: t7814, gLong, sr, sec, tinf, zz,  cd9, cd11, sigma
...
!v10      T7 =  ( -0.105531 *PLG(2,2) - 0.607134E-2 *PLG(4,2) + T71 )  *CTLOC
!v10     4     + ( -0.115622 *PLG(2,2) + 0.202240E-2 *PLG(4,2) + T72 ) *STLOC

      t7   =  ( -0.105531 *PLG(2,2) - 0.607134E-2 *PLG(4,2) + T71 )
      t7   = t7 * ctLoc
      t7xxx=  ( -0.115622 *PLG(2,2) + 0.202240E-2 *PLG(4,2) + T72 ) 
      t7xxx= t7xxx *stLoc
      t7   = t7 + t7xxx
...
!v10      T14 = (0.147284E-2 *PLG(4,4) -0.173933E-3*Z1 +0.365016E-4*Z2)*S3TLOC
!v10     2     +(0.341345E-3 *PLG(4,4) -0.153218E-3*Z1 +0.115102E-3*Z2)*C3TLOC 

      t14 =   (0.147284E-2 *PLG(4,4) -0.173933E-3*Z1 +0.365016E-4*Z2) 
      t14 =       t14 * s3tLoc
      xxxt14 = (0.341345E-3 *PLG(4,4) -0.153218E-3*Z1 +0.115102E-3*Z2)
      xxxt14 = xxxt14 * c3tLoc
      t14 =t14+xxxt14
...
C     to the Huron Messaging System email list, send a request to hur_parent@aaps.k12.mi.us Include your name, email address, student name(s), and grade(s) in the email and please include "Huron Messaging System" in the title.
...
c----------------------------------------------------------------
       ...
       double precision   ARGMAX
       parameter (        ARGMAX=87.0  )
        real :: dxdh, x0,xmx0,x, eptr1, eptr2, y
        real :: xxxarg,h
        xxxarg = argmax 
        h =hx              
      ...
      y = BETA * ETA * eptr1 + ZETA * (100. * eptr2 - xmx0)
        ...
        Y = y * dxdh
       if(abs(Y) >  xxxarg) Y = sign(xxxarg,Y)
...
C
      real FUNCTION DXE1N(Hx)                            
       use EUA_BLo10
       use EUA_BLoCK1
C LOGARITHMIC DERIVATIVE OF FUNCTION XE1 (KM-1).   
        implicit NONE
        real, intent(in):: Hx
        real :: h , x,x0, epst1,epst2
        h=hx

       x0 = 300.0d0 - delta
       X=(H -HMF2)/(1000.0d0 -HMF2)*700.0d0 + x0
       epst2 = epst(x, 100.0, 300.0)
       epst1 = epst(x, beta , 394.5)
      DXE1N = - ETA * epst1 + ZETA * (1.d0 - epst2)             
      RETURN          
      END              FUNCTION DXE1N
C*****
...
C
      REAL FUNCTION XE6(Hx)                         
C ELECTRON DENSITY FOR THE D REGION (HA...HME).    
      use       EUA_BLOCK4, XNME=>NME  !/HME,NME,HEF  
      use       EUA_BLOCK6, XNMD=>NMD  !/HMD,NMD,HDX   
      use       EUA_BLOCK7             !/D1,XKK,FP30,FP3U,FP1,FP2
      ...
      IF(H.GT.HDX)  then         ! GOTO 100                        
100     Z   =  HME -H 
        xx  =  z**XKK
        xx  = -d1*xx       
       XE6  =  XNME*EXP(xx)
c        XE6 = XNME*EXP(-D1*Z**XKK)
...
c        XE6 =XNMD*EXP(Z*(FP1+Z*(FP2+Z*FP3)))           
...
C ELECTRON DENSITY FOR THE INDERMEDIUM REGION (HEF..HZ).    
      ...
      IF(HST <  0.)    then  !   GOTO 100                       
100       XE4=XNME+T*(H-HEF)      
      else 
          xx = T*(HZ -H +T/4.0d0)
          xx = SQRT(xx)*SIGN(1.0 ,T)
          yy = HZ + T/2.0d0 
          xx = yy - xx
...
!v10> C
!v10>       REAL FUNCTION ELTE(H)
!v10> c----------------------------------------------------------------
!v10> C ELECTRON TEMPERATURE PROFILE BASED ON THE TEMPERATURES AT 120                 
!v10> C HMAX,300,400,600,1400,3000 KM ALTITUDE. INBETWEEN CONSTANT                    
!v10> C GRADIENT IS ASSUMED. ARGMAX IS MAXIMUM ARGUMENT ALLOWED FOR
!v10> C EXP-FUNCTION.
!v10> c----------------------------------------------------------------
!v10>       use         EUA_BLOTE, ah=>ahh, st=>stte, d=>dte 
!v10>                           !/AHH,  ATE1, STTE ,DTE
!v10> cv10             /EUA_BLOTE/AH(7),ATE1, ST(6),D(5)
!v10>       implicit none
!v10>        real, intent(in):: h
!v10>        real            :: sum, aa, bb, xxdi, xxahi, xxah1
!v10>        integer         :: i
!v10> C
!v10>       SUM=ATE1+ST(1)*(H-AH(1))                     
!v10>       DO  I=1,5    !1
!v10>           xxdi=d(i)
!v10>           xxahi=ah(i+1)
!v10>           xxah1=ah(1)
!v10> cv10      aa = eptr(h    ,d(i),ah(i+1))
!v10>           aa = eptr(h    ,xxdi,xxahi)
!v10> 
!v10> cv10      bb = eptr(ah(1),d(i),ah(i+1))       
!v10>           bb = eptr(xxah1,xxdi,xxahi)
!v10> 
!v10> cv101         SUM=SUM+(ST(I+1)-ST(I))*(AA-BB)*D(I)     
!v10> 1         SUM=SUM+(ST(I+1)-ST(I))*(AA-BB)*xxDI                
!v10> 
!v10>       end do
!v10>  
!v10>       ELTE=SUM        
!v10>       RETURN          
!v10>       END              FUNCTION ELTE
!v10> C*****
!v10> 
...
!v10> 
!v10> C
!v10>       real FUNCTION TEDE(H,DEN,COV)                     
!v10>        implicit NONE
!v10> C ELECTRON TEMEPERATURE MODEL AFTER BRACE,THEIS .  
!v10> C FOR NEG. COV THE MEAN COV-INDEX (3 SOLAR ROT.) IS EXPECTED.  
!v10> C DEN IS THE ELECTRON DENSITY IN M-3.  
!v10>        real, intent(in) :: h, den, cov
!v10>        real  :: y, yc, acov
!v10>        real  :: xx , yy     
!v10>             
!v10>         xx = (-5.122E-4*H+(6.094E-12-3.353E-14*H)*DEN) 
!v10>         yy =  1051.+(17.01*H-2746.)
!v10>       Y  = yy*EXP(xx)        
!v10> 
!v10>       ACOV=ABS(COV)   
!v10> 
!v10>       IF(COV.LT.0.)   then
!v10>         xx = -(ACOV  -115.0d0)/10.d0
!v10>         xx = 1.d0    + EXP(xx)
!v10>         yy = 0.123d0 + 1.69d-3*ACOV
!v10>       else
!v10>         xx = -(ACOV  -102.5d0)/5.d0
!v10>         xx = 1.0d0   +EXP(xx)  
!v10>         yy = 0.117d0 +2.02d-3*ACOV
!v10>       end if
!v10> 
!v10>       YC  = 1.0d0  +yy/xx
!v10> 
...
!cv10> C                     
!cv10> C*************************************************************                  
!cv10> C**************** ION TEMPERATURE ****************************
!cv10> C*************************************************************                  
!cv10> C
!cv10> C
!cv10>       REAL FUNCTION TI(H)
!cv10> c----------------------------------------------------------------
!cv10> C ION TEMPERATURE FOR HEIGHTS NOT GREATER 1000 KM AND NOT LESS HS               
!cv10> C EXPLANATION SEE FUNCTION RPID.                   
!cv10> c----------------------------------------------------------------
!cv10>       use     EUA_BLOCK8, G=>DTI, M=>MXSM  !/HS,TNHS,XSM, MM, DTI, MXSM 
!cv10>       implicit none
!cv10> 
!cv10> !      REAL               MM
!cv10> !      /EUA_BLOCK8/        HS,TNHS,XSM(4),MM(5),G(4),M
!cv10> 
!cv10>       real, intent(in):: h
!cv10>       real   :: sum,aa,bb, xxgi, xxsmi
!cv10>       integer:: i  
!cv10> 
!cv10>       SUM=MM(1)*(H-HS)+TNHS                        
!cv10>       DO  I=1,M-1  !100
!cv10>         xxgi =g(i)
!cv10>         xxsmi=xsm(i)
!cv10> cv10       aa = eptr(h ,g(i),xsm(i))
!cv10>            aa = eptr(h ,xxgi,xxsmi)
!cv10> 
!cv10> cv10       bb = eptr(hs,g(i),xsm(i))
!cv10>            bb = eptr(hs,xxgi,xxsmi)
!cv10> 
!cv10> cv10 100        SUM=SUM+(MM(I+1)-MM(I))*(AA-BB)*G(I)                
!cv10> 100        SUM=SUM+(MM(I+1)-MM(I))*(AA-BB)*xxGI                
!cv10>       end do
!cv10>     
!cv10>       TI=SUM          
!cv10>       RETURN          
!cv10>       END              FUNCTION TI
!cv10> C*****
!cv10> 
!cv10> 
...
C THE  HEIGHT ABOVE WHICH TN BEGINS TO BE DIFFERENT FROM TI      
      use       EUA_BLOTN,tex=>texos,tlbd=>tlbdh,sig=>sigma  
      implicit none
      real, intent(in) :: h
      real             :: tnh, dtdx

                    !/XSM1,TEXOS,TLBDH,SIGMA 
cv10       /EUA_BLOTN/XSM1,TEX  ,TLBD ,SIG
...
!v10>              
!v10> C*************************************************************         
!v10> C************* ION RELATIVE PRECENTAGE DENSITY *****************     
!v10> C*************************************************************    
!v10> C
!v10> C
!v10>       REAL FUNCTION RPID (H, H0, N0, M, ST, ID, XS)
!v10> c------------------------------------------------------------------
!v10> C D.BILITZA,1977,THIS ANALYTIC FUNCTION IS USED TO REPRESENT THE      
!v10> C RELATIVE PRECENTAGE DENSITY OF ATOMAR AND MOLECULAR OXYGEN IONS.    
!v10> C THE M+1 HEIGHT GRADIENTS ST(M+1) ARE CONNECTED WITH EPSTEIN-     
!v10> C STEP-FUNCTIONS AT THE STEP HEIGHTS XS(M) WITH TRANSITION        
!v10> C THICKNESSES ID(M). RPID(H0,H0,N0,....)=N0.       
!v10> C ARGMAX is the highest allowed argument for EXP in your system.
!v10> c------------------------------------------------------------------
!v10>        implicit none
!v10>  
!v10>       REAL, intent(in)      :: h, h0,   N0, st,xs         
!v10>       integer ,intent(in)   :: m, id
!v10>       DIMENSION        ID(*), ST(*), XS(*)       
!v10> 
!v10>       real    :: sum,aa,bb, xxsi, xi,sm
!v10>       integer :: i        
!v10> 
!v10> 
!v10> cv10        /EUA_ARGEXP/       ARGMAX
!v10>        double precision        ARGMAX
!v10>        parameter (        ARGMAX=87.0            )
!v10> 
!v10> 
!v10>       SUM=(H-H0)*ST(1)                             
!v10>       DO   I=1,M   ! 100
!v10>              XI  =ID(I)
!v10>              xxsi=xs(i)
!v10> !v10          aa = eptr(h ,xi,xs(i))
!v10>               aa = eptr(h ,xi,xxsi)
!v10> 
!v10> !v10          bb = eptr(h0,xi,xs(i))
!v10>               bb = eptr(h0,xi,xxsi)
!v10> 
!v10> 
!v10> 100             SUM=SUM+(ST(I+1)-ST(I))*(AA-BB)*XI 
!v10>       end do
!v10> 
!v10>       IF(ABS(SUM) .LT. ARGMAX) then
!v10>        SM=EXP(SUM)
!v10>       else IF(SUM .Gt. 0.0d0) then
!v10>        SM=EXP(ARGMAX)
!v10>       else
!v10>        SM=0.0d0
!v10>       endif
...
!v10> 
!v10> C
!v10>       SUBROUTINE  KOEFP1(PG1O)                     
!v10> C THIEMANN,1979,COEFFICIENTS PG1O FOR CALCULATING  O+ PROFILES    
!v10> C BELOW THE F2-MAXIMUM. CHOSEN TO APPROACH DANILOV-          
!v10> C SEMENOV'S COMPILATION.                           
!v10>       implicit  none
!v10>       real, DIMENSION(80) :: PG1O                           
!v10>       REAL FELD (80)  
!v10>       DATA FELD/-11.0,-11.0,4.0,-11.0,0.08018,     
!v10>      &0.13027,0.04216,0.25  ,-0.00686,0.00999,     
!v10>      &5.113,0.1 ,170.0,180.0,0.1175,0.15,-11.0,    
!v10>      &1.0 ,2.0,-11.0,0.069,0.161,0.254,0.18,0.0161,     
!v10>      &0.0216,0.03014,0.1,152.0,167.0,0.04916,      
!v10>      &0.17,-11.0,2.0,2.0,-11.0,0.072,0.092,0.014,0.21,    
!v10>      &0.01389,0.03863,0.05762,0.12,165.0,168.0,0.008,     
!v10>      &0.258,-11.0,1.0,3.0,-11.0,0.091,0.088,       
!v10>      &0.008,0.34,0.0067,0.0195,0.04,0.1,158.0,172.0,     
!v10>      &0.01,0.24,-11.0,2.0,3.0, -11.0,0.083,0.102,  
!v10>      &0.045,0.03,0.00127,0.01,0.05,0.09,167.0,185.0,      
!v10>      &0.015,0.18/     
!v10> 
!v10> !      K=0             
!v10> !      DO 10 I=1,80    
!v10> !      K=K+1           
!v10> !10    PG1O(K)=FELD(I)                              
!v10>       
!v10>       pg1o =feld     
!v10> 
!v10>       RETURN          
!v10>       END              SUBROUTINE KOEFP1
!v10> C*****
!v10> 
!v10> C
!v10>       SUBROUTINE KOEFP2(PG2O)                      
!v10> C THIEMANN,1979,COEFFICIENTS FOR CALCULATION OF O+ PROFILES                     
!v10> C ABOVE THE F2-MAXIMUM (DUMBS,SPENNER:AEROS-COMPILATION)                        
!v10>       implicit none
!v10>       real, DIMENSION(32)  ::  PG2O
!v10>       REAL FELD(32)   
!v10>       DATA FELD/1.0,-11.0,-11.0,1.0,695.0,-.000781,        
!v10>      &-.00264,2177.0,1.0,-11.0,-11.0,2.0,570.0,    
!v10>      &-.002,-.0052,1040.0,2.0,-11.0,-11.0,1.0,695.0,      
!v10>      &-.000786,-.00165,3367.0,2.0,-11.0,-11.0,2.0, 
!v10>      &575.0,-.00126,-.00524,1380.0/                
!v10> !      K=0             
!v10> !      DO 10 I=1,32    
!v10> !      K=K+1           
!v10> !10    PG2O(K)=FELD(I)                              
!v10>        
!v10>        pg2o = feld
!v10> 
!v10>       RETURN          
!v10>       END              SUBROUTINE KOEFP2
!v10> C*****
!v10> C
!v10>       SUBROUTINE  KOEFP3(PG3O)                     
!v10> C THIEMANN,1979,COEFFICIENTS FOR CALCULATING O2+ PROFILES.      
!v10> C CHOSEN AS TO APPROACH DANILOV-SEMENOV'S COMPILATION.       
!v10>       implicit none
!v10>       real, DIMENSION(80) :: PG3O
!v10>       REAL FELD(80)   
!v10>       DATA FELD/-11.0,1.0,2.0,-11.0,160.0,31.0,130.0,         
!v10>      &-10.0,198.0,0.0,0.05922,-0.07983,            
!v10>      &-0.00397,0.00085,-0.00313,0.0,-11.0,2.0,2.0,-11.0,         
!v10>      &140.0,30.0,130.0,-10.0,                      
!v10>      &190.0,0.0,0.05107,-0.07964,0.00097,-0.01118,-0.02614,      
!v10>      &-0.09537,       
!v10>      &-11.0,1.0,3.0,-11.0,140.0,37.0,125.0,0.0,182.0,       
!v10>      &0.0,0.0307,-0.04968,-0.00248,                
!v10>      &-0.02451,-0.00313,0.0,-11.0,2.0,3.0,-11.0,   
!v10>      &140.0,37.0,125.0,0.0,170.0,0.0,              
!v10>      &0.02806,-0.04716,0.00066,-0.02763,-0.02247,-0.01919,     
!v10>      &-11.0,-11.0,4.0,-11.0,140.0,45.0,136.0,-9.0, 
!v10>      &181.0,-26.0,0.02994,-0.04879,                
!v10>      &-0.01396,0.00089,-0.09929,0.05589/           
!v10> !      K=0             
!v10> !      DO 10 I=1,80    
!v10> !      K=K+1           
!v10> !10    PG3O(K)=FELD(I)                              
!v10> 
!v10>        pg3o =feld
!v10>  
!v10>       RETURN          
!v10>       END              SUBROUTINE KOEFP3
!v10> C*****
!v10> 
!v10> 
!v10> 
...
!v10> 
!v10> C
!v10>       SUBROUTINE SUFE (FIELD,RFE,M,FE)             
!v10> C SELECTS THE REQUIRED ION DENSITY PARAMETER SET.
!v10> C THE INPUT FIELD INCLUDES DIFFERENT SETS OF DIMENSION M EACH    
!v10> C CARACTERISED BY 4 HEADER NUMBERS. RFE(4) SHOULD CONTAIN THE   
!v10> C CHOSEN HEADER NUMBERS.FE(M) IS THE CORRESPONDING SET.        
!v10>       implicit none
!v10>       integer, intent(in) :: m
!v10>       real                :: rfe,   fe,   field   , efe
!v10>       DIMENSION RFE(4),FE(*),FIELD(*),EFE(4)
!v10>       
!v10>       integer :: k, i      
!v10>      
!v10>        K=0             
!v10> 100   DO  I=1,4     !101
!v10>        K =K+1           
!v10> 101    EFE(I)=FIELD(K)
!v10>       end do                              
!v10> 
!v10>       DO  I=1,M  !111
!v10>        K=K+1           
!v10> 111    FE(I)=FIELD(K) 
!v10>       end do 
!v10> 
!v10>       DO  I=1,4    !120
!v10>        IF((EFE(I) .GT. -10.0) .AND. ( RFE(I).NE.EFE(I))) GOTO 100     
!v10> 120   CONTINUE        
!v10>       end do
!v10> 
!v10>       RETURN          
!v10>       END              SUBROUTINE SUFE
!v10> C****
!v10> 
!v10> 
...
C SHEIKH,4.3.77.      
      ...
      real, intent(in)   :: smodip, sLat, sLong, hour, sfe
      integer, intent(in):: iharm, nq, k1, m,mm,m3   
      
      real    :: hou, ss, s3, xsinx, s0, s1,s2
      integer :: i,mi,j, index, np, L

!v10>      REAL*8 
      double precision ::C(12),S(12),COEF(100),SUM      
...
!v10> 
!v10> C
!v10>       SUBROUTINE GGM(ART,LONG,LATI,MLONG,MLAT)            
!v10>        implicit NONE
!v10> C CALCULATES GEOMAGNETIC LONGITUDE (MLONG) AND LATITUDE (MLAT) 
!v10> C FROM GEOGRAFIC LONGITUDE (LONG) AND LATITUDE (LATI) FOR ART=0
!v10> C AND REVERSE FOR ART=1. ALL ANGLES IN DEGREE.
!v10> C LATITUDE:-90 TO 90. LONGITUDE:0 TO 360 EAST.         
!v10> 
!v10>       INTEGER, intent(in):: ART     
!v10> c      REAL, intent(in)   :: MLONG !, MLAT,LONG
!v10>       real, intent(in out)  :: mLong,Lati,Long,mLat
!v10> cv10      /EUA_CONST/FAKTOR
!v10>        double precision   FAKTOR 
!v10>        parameter (        FAKTOR = 1.7453292519943295D-02 )
!v10> 
!v10>        real :: zpi,cbg, ci, si,cbm, sbm
!v10>        real :: cLm, sLm, sbg, yLg, cLg,sLg
!v10> 
!v10> 
!v10>       ZPI =FAKTOR*360.                              
!v10>       CBG =11.4*FAKTOR                              
...
!v10>       SI =SIN(CBG)
!v10> 
!v10>       IF(ART.EQ.0) GOTO 10                         
!v10>       CBM = COS( MLAT *FAKTOR )                           
!v10>       SBM = SIN( MLAT *FAKTOR )                           
!v10>       CLM = COS( MLONG*FAKTOR )                          
!v10>       SLM = SIN( MLONG*FAKTOR )
!v10>       SBG = SBM*CI -CBM*CLM*SI
!v10> 
!v10>         IF(ABS(SBG).GT.1.) SBG=SIGN(1.,SBG)
!v10>       LATI = ASIN(SBG)
!v10>       CBG = COS(LATI)     
!v10>       SLG =(CBM*SLM)/CBG  
!v10>       CLG =(SBM*SI+CBM*CLM*CI)/CBG
!v10>         IF(ABS(CLG).GT.1.) CLG=SIGN(1.,CLG)     
!v10>       LONG =ACOS(CLG)  
!v10>       IF(SLG.LT.0.0) LONG=ZPI-LONG
!v10>       LATI = LATI/FAKTOR    
!v10>       LONG = LONG/FAKTOR  
!v10>       LONG = LONG-69.8    
!v10>       IF(LONG.LT.0.0) LONG=LONG+360.0                 
!v10>       RETURN          
!v10> 
!v10> 10    YLG =LONG+69.8    
!v10>       CBG = COS(LATI*FAKTOR)                           
!v10>       SBG = SIN(LATI*FAKTOR)                           
!v10>       CLG = COS(YLG*FAKTOR)                          
!v10>       SLG = SIN(YLG*FAKTOR)                          
!v10>       SBM = SBG*CI+CBG*CLG*SI                        
!v10>         IF(ABS(SBM).GT.1.) SBM=SIGN(1.,SBM)
!v10>       MLAT =ASIN(SBM)   
!v10>       CBM =COS(MLAT)     
!v10>       SLM =(CBG*SLG)/CBM                            
!v10>       CLM =(-SBG*SI+CBG*CLG*CI)/CBM
!v10>         IF(ABS(CLM).GT.1.) CLM=SIGN(1.,CLM) 
!v10>       MLONG = ACOS(CLM)
!v10>       IF(SLM.LT..0) MLONG=ZPI-MLONG
!v10>       MLAT  = MLAT /FAKTOR    
!v10>       MLONG = MLONG/FAKTOR  
!v10>       RETURN          
!v10>       END              SUBROUTINE GGM
!v10> C
!v10> 
!v10> 
!v10> 
...
!v10> 
!v10> C
!v10>       SUBROUTINE FIELDG(DLAT,DLONG,ALT,X,Y,Z,F,DIP,DEC,SMODIP)   
!v10> 
!v10> C THIS IS A SPECIAL VERSION OF THE POGO 68/10 MAGNETIC FIELD      
!v10> C LEGENDRE MODEL. TRANSFORMATION COEFF. G(144) VALID FOR 1973.   
!v10> C INPUT: DLAT, DLONG=GEOGRAPHIC COORDINATES/DEG.(-90/90,0/360),    
!v10> C        ALT=ALTITUDE/KM.                          
!v10> C OUTPUT: F TOTAL FIELD (GAUSS), Z DOWNWARD VERTICAL COMPONENT       
!v10> C        X,Y COMPONENTS IN THE EQUATORIAL PLANE (X TO ZERO LONGITUDE).   
!v10> C        DIP INCLINATION ANGLE(DEGREE). SMODIP RAWER'S MODFIED DIP.     
!v10> C SHEIK,1977.         
!v10>         
!v10>        implicit none
!v10>       real, intent (in)   :: dLat, dLong, aLt  
!v10> 
!v10>       real, intent(out)   :: x,y,z, f
!v10>       real, intent(out)   :: dip, dec,smodip
!v10> 
!v10>       
!v10>       integer :: k, i, nmax, ih,iL, ihmax, Last, imax, m
!v10>       real    :: feL1, feL2,g, rLat,rLong,  ct, st, d,cp,sp,h,s
!v10>       real    :: zzz, rho, xxx, yyy, rq, xi, f1, x1, y1, z1,xt
!v10>       real    :: brh0, zdivf,  ydivs, dipdiv
!v10> 
!v10>       DIMENSION H(144),XI(3),G(144),FEL1(72),FEL2(72)
!v10> cv10     /EUA_CONST/UMR 
!v10>        double precision       UMR 
!v10>        parameter ( UMR= 1.7453292519943295D-02 )
!v10> 
!v10>                           
!v10>       DATA FEL1/0.0, 0.1506723,0.0101742, -0.0286519, 0.0092606,    
!v10>      & -0.0130846, 0.0089594, -0.0136808,-0.0001508, -0.0093977,   
!v10>      & 0.0130650, 0.0020520, -0.0121956, -0.0023451, -0.0208555,   
!v10>      & 0.0068416,-0.0142659, -0.0093322, -0.0021364, -0.0078910,   
!v10>      & 0.0045586,  0.0128904, -0.0002951, -0.0237245,0.0289493,   
!v10>      & 0.0074605, -0.0105741, -0.0005116, -0.0105732, -0.0058542,  
!v10>      &0.0033268, 0.0078164,0.0211234, 0.0099309, 0.0362792,     
!v10>      &-0.0201070,-0.0046350,-0.0058722,0.0011147,-0.0013949,     
!v10>      & -0.0108838,  0.0322263, -0.0147390,  0.0031247, 0.0111986,   
!v10>      & -0.0109394,0.0058112,  0.2739046, -0.0155682, -0.0253272,   
!v10>      &  0.0163782, 0.0205730,  0.0022081, 0.0112749,-0.0098427,   
!v10>      & 0.0072705, 0.0195189, -0.0081132, -0.0071889, -0.0579970,    
!v10>      & -0.0856642, 0.1884260,-0.7391512, 0.1210288, -0.0241888,    
!v10>      & -0.0052464, -0.0096312, -0.0044834, 0.0201764,  0.0258343,   
!v10>      &0.0083033,  0.0077187/                       
!v10>       DATA FEL2/0.0586055,0.0102236,-0.0396107,    
!v10>      & -0.0167860, -0.2019911, -0.5810815,0.0379916,  3.7508268,   
!v10>      & 1.8133030, -0.0564250, -0.0557352, 0.1335347, -0.0142641,   
!v10>      & -0.1024618,0.0970994, -0.0751830,-0.1274948, 0.0402073,    
!v10>      &  0.0386290, 0.1883088,  0.1838960, -0.7848989,0.7591817,    
!v10>      & -0.9302389,-0.8560960, 0.6633250, -4.6363869, -13.2599277,   
!v10>      & 0.1002136,  0.0855714,-0.0991981, -0.0765378,-0.0455264,    
!v10>      &  0.1169326, -0.2604067, 0.1800076, -0.2223685, -0.6347679,   
!v10>      &0.5334222, -0.3459502,-0.1573697,  0.8589464, 1.7815990,    
!v10>      &-6.3347645, -3.1513653, -9.9927750,13.3327637, -35.4897308,   
!v10>      &37.3466339, -0.5257398,  0.0571474, -0.5421217,  0.2404770,   
!v10>      & -0.1747774,-0.3433644, 0.4829708,0.3935944, 0.4885033,    
!v10>      &  0.8488121, -0.7640999, -1.8884945, 3.2930784,-7.3497229,    
!v10>      & 0.1672821,-0.2306652, 10.5782146, 12.6031065, 8.6579742,     
!v10>      & 215.5209961, -27.1419220,22.3405762,1108.6394043/         
!v10>      
!v10>       K=0             
!v10>       DO I=1,72      !10
!v10>        K=K+1           
!v10>        G(K)=FEL1(I)    
!v10> 10     G(72+K)=FEL2(I)         
!v10>       end do      
!v10> 
!v10>       RLAT=DLAT*UMR   
!v10>       CT=SIN(RLAT)    
!v10>       ST=COS(RLAT)    
!v10>       NMAX=11         
!v10>       D=SQRT(40680925.0-272336.0*CT*CT)   
!v10>       RLONG=DLONG*UMR                   
!v10>       CP=COS(RLONG)   
!v10>       SP=SIN(RLONG)   
!v10>       ZZZ=(ALT+40408589.0/D)*CT/6371.2      
!v10>       RHO=(ALT+40680925.0/D)*ST/6371.2    
!v10>       XXX=RHO*CP      
!v10>       YYY=RHO*SP      
!v10>       RQ=1.0/(XXX*XXX+YYY*YYY+ZZZ*ZZZ)    
!v10>       XI(1)=XXX*RQ    
!v10>       XI(2)=YYY*RQ    
!v10>       XI(3)=ZZZ*RQ    
!v10>       IHMAX=NMAX*NMAX+1          
!v10>       LAST=IHMAX+NMAX+NMAX       
!v10>       IMAX=NMAX+NMAX-1            
!v10>       DO 100 I=IHMAX,LAST       
!v10> 100   H(I)=G(I)       
!v10>       DO 200 K=1,3,2  
!v10>       I=IMAX          
!v10>       IH=IHMAX        
...
!v10>       Z1=XI(3)*(F1+F1)          
!v10>       I=I-2           
!v10>       IF((I-1).LT.0) GOTO 400         
!v10>       IF((I-1).EQ.0) GOTO 500         
!v10>       DO 600 M=3,I,2  
!v10>       H(IL+M+1)=G(IL+M+1)+Z1*H(IH+M+1)+X1*(H(IH+M+3)-H(IH+M-1))-    
!v10>      &Y1*(H(IH+M+2)+H(IH+M-2))                     
!v10>       H(IL+M)=G(IL+M)+Z1*H(IH+M)+X1*(H(IH+M+2)-H(IH+M-2))+     
!v10>      &Y1*(H(IH+M+3)+H(IH+M-1))                     
!v10> 600   CONTINUE        
!v10> 500   H(IL+2)=G(IL+2)+Z1*H(IH+2)+X1*H(IH+4)-Y1*(H(IH+3)+H(IH))    
!v10>       H(IL+1)=G(IL+1)+Z1*H(IH+1)+Y1*H(IH+4)+X1*(H(IH+3)-H(IH))   
!v10> 400   H(IL)=G(IL)+Z1*H(IH)+2.0*(X1*H(IH+1)+Y1*H(IH+2))           
!v10> 700   IH=IL           
!v10>       IF(I.GE.K) GOTO 300                          
!v10> 200   CONTINUE        
!v10>       S=0.5*H(1)+2.0*(H(2)*XI(3)+H(3)*XI(1)+H(4)*XI(2))     
!v10>       XT=(RQ+RQ)*SQRT(RQ)          
!v10>       X=XT*(H(3)-S*XXX)            
!v10>       Y=XT*(H(4)-S*YYY)             
!v10>       Z=XT*(H(2)-S*ZZZ)             
!v10>       F=SQRT(X*X+Y*Y+Z*Z)           
!v10>       BRH0=Y*SP+X*CP  
!v10>       Y=Y*CP-X*SP     
!v10>       X=Z*ST-BRH0*CT  
!v10>       Z=-Z*CT-BRH0*ST 
!v10>        zdivf=z/f
!v10>         IF(ABS(zdivf).GT.1.) zdivf=SIGN(1.,zdivf)
!v10>       DIP=ASIN(zdivf)
!v10>        ydivs=y/sqrt(x*x+y*y)  
!v10>         IF(ABS(ydivs).GT.1.) ydivs=SIGN(1.,ydivs)
!v10>       DEC=ASIN(ydivs)
!v10>        dipdiv=DIP/SQRT(DIP*DIP+ST)
!v10>         IF(ABS(dipdiv).GT.1.) dipdiv=SIGN(1.,dipdiv)
!v10>       SMODIP=ASIN(dipdiv)
!v10>       DIP=DIP/UMR     
!v10>       DEC=DEC/UMR     
!v10>       SMODIP=SMODIP/UMR          
!v10>       RETURN          
!v10>       END              SUBROUTINE FIELDG
!v10> C*****
!v10> 
!v10> 
...
C
      ...
      SUBROUTINE REGFA1(X11,X22,FX11,FX22,EPS,FW,F,SCHALT,Xres) 
...
C THEN SCHALT=.TRUE.  
      ...
      real, intent(in)   :: x11,x22, fx11,fx22, EPS, fw
      real, intent(out)  :: Xres
      LOGICAL :: L1,LINKS,K,SCHALT         
      real    :: ep, x1,x2, f1,f2, dx,fx
      real    :: x
      integer :: ng, Lfd  
      ...
      DO    !zzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzz
...
300      L1 = LINKS        
         ...
         IF((LINKS.AND.(.NOT.L1)).OR.(.NOT.LINKS.AND.L1)) then
            NG=2*NG  
         end if
...
400   FX  = F(X)-FW
      ...
      IF( ABS(X2-X1) <= EP)  EXIT      
      END DO  !zzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzz   

      Xres = x
      RETURN          
      END              SUBROUTINE REGFA1
C
c\\\\\\\\\\\\\\\\01/03/08 work with -O1 correctly \\\\\\\\\\\\\\\\\\
...
c\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
C
      SUBROUTINE REGFA1zz(X11,X22,FX11,FX22,EPS,FW,F,SCHALT,Xres) 
C REGULA-FALSI-PROCEDURE TO FIND X WITH F(X)-FW=0. X1,X2 ARE THE        
C STARTING VALUES. THE COMUTATION ENDS WHEN THE X-INTERVAL            
C HAS BECOME LESS THAN EPS . IF SIGN(F(X1)-FW)= SIGN(F(X2)-FW)      
C THEN SCHALT=.TRUE.  
...
c
       ...
       parameter ( dtr= 1.7453292519943295D-02 )

       integer, intent(in) :: Ld
       real, intent(in)    :: t,flat, elon
       real, intent(out)   :: declin, zenith, sunrse, sunset

       real :: wlon, td, te, dcL, p1, p2, p3, p4, p6, dc
       real :: tf, eqt, et, fa, phi, a, b, cosx, ch
       real :: cosphi, secphi
...
!v10> 
!v10> C
!v10>       real FUNCTION HPOL(HOUR,TW,XNW,SA,SU,DSA,DSU)            
!v10>        implicit NONE
!v10> C-------------------------------------------------------
!v10> C PROCEDURE FOR SMOOTH TIME-INTERPOLATION USING EPSTEIN  
!v10> C STEP FUNCTION AT SUNRISE (SA) AND SUNSET (SU). THE 
!v10> C STEP-WIDTH FOR SUNRISE IS DSA AND FOR SUNSET DSU.
!v10> C TW,NW ARE THE DAY AND NIGHT VALUE OF THE PARAMETER TO 
!v10> C BE INTERPOLATED. SA AND SU ARE TIME OF SUNRIES AND 
!v10> C SUNSET IN DECIMAL HOURS.
!v10> C BILITZA----------------------------------------- 1979.
!v10>         real, intent(in):: Hour,tw,xnw, sa, su,dsa,dsu
!v10>         real ::xx,yy
!v10>       
!v10> 
!v10>         IF(ABS(SU).GT.25.) THEN
!v10>               IF(SU.GT.0.0) THEN
!v10>                      HPOL=TW
!v10>               ELSE
!v10>                      HPOL=XNW
!v10>               ENDIF
!v10>               RETURN
!v10>         ENDIF
!v10>         xx  = (XNW-TW)*EPST(HOUR,DSU,SU) 
!v10>         yy  = (TW-XNW)*EPST(HOUR,DSA,SA)
!v10>         HPOL= XNW +yy +xx
!v10>  
!v10> c      HPOL=XNW+(TW-XNW)*EPST(HOUR,DSA,SA)+
!v10> c     &         (XNW-TW)*EPST(HOUR,DSU,SU) 
!v10> 
!v10>       RETURN          
!v10>       END        FUNCTION HPOL
!v10> C*****      
!v10> 
!v10> 
...
!v10> 
!v10> C
!v10>        SUBROUTINE MODA(IN,MONTH,IDAY,IDOY)
!v10>        implicit NONE
!v10> C-------------------------------------------------------------------
!v10> C CALCULATES DAY OF YEAR (IDOY) FROM MONTH (MONTH) AND DAY (IDAY) 
!v10> C IF IN=0, OR MONTH (MONTH) AND DAY (IDAY) FROM DAY OF 
!v10> C YEAR (IDOY), IF IN=1. 
!v10> C-------------------------------------------------------------------
!v10>        integer, intent(in) :: in
!v10>        integer, intent(in out)::  month,  iday, idoy
!v10>        integer :: imo, mobe, mo, mooLd
!v10> 
!v10>        DIMENSION MO(0:12)
!v10>        DATA      MO/0,31,59,90,120,151,181,212,243,273,304,334,365/
!v10> 
!v10>        IMO =0
!v10>        MOBE=0
!v10> 
!v10>        IF(IN > 0) GOTO 5
!v10>           IDOY  =MO(MONTH)+IDAY
!v10>           RETURN
!v10> 
!v10> 5       IMO   = IMO+1
!v10>         MOOLD = MOBE
!v10>         IF(IMO  > 12)   GOTO 55
!v10>            MOBE  = MO(IMO)
!v10>         IF(MOBE < IDOY) GOTO 5
!v10> 
!v10> CCC       used to be IMO-1 which was just plain wrong
!v10> 
!v10> 55       MONTH=IMO
!v10>          IDAY =IDOY-MOOLD
!v10>        RETURN
!v10>        END               SUBROUTINE MODA
!v10> c
!v10> 
!v10>  
!v10> 
...
!v10> 
!v10> C
!v10>        REAL FUNCTION B0POL ( HOUR, SAX, SUX, ISEASON, R, DELA)
!v10> C-----------------------------------------------------------------
!v10> C Interpolation procedure for bottomside thickness parameter B0.
!v10> C Array B0F(ILT,ISEASON,IR,ILATI) distinguishes between day and
!v10> C night (ILT=1,2), four seasons (ISEASON=1 spring), low and high
!v10> C solar activity (IR=1,2), and low and middle modified dip
!v10> C latitudes (ILATI=1,2). In the DATA statement the first value
!v10> C corresponds to B0F(1,1,1,1), the second to B0F(2,1,1,1), the
!v10> C third to B0F(1,2,1,1) and so on.
!v10> C JUNE 1989 --------------------------------------- Dieter Bilitza
!v10> C
!v10>       implicit none
!v10>      
!v10>        real, intent(in)    :: hour, sax, sux, r, dela
!v10>        integer, intent(in) :: iseason
!v10> 
!v10>        real    :: b0f, siph, sipL, dayvaL
!v10>        integer :: isr, isL
!v10> 
!v10> 
!v10>       REAL              NITVAL
!v10>       DIMENSION        B0F(2,4,2,2),SIPH(2),SIPL(2)
!v10>       DATA        B0F/114.,64.0,134.,77.0,128.,66.0,75.,73.0,
!v10>      &                  113.,115.,150.,116.,138.,123.,94.,132.,
!v10>      &                  72.0,84.0,83.0,89.0,75.0,85.0,57.,76.0,
!v10>      &                  102.,100.,120.,110.,107.,103.,76.,86.0/
!v10> 
!v10>          DO  ISR=1,2      !7034
!v10>             DO ISL=1,2    !7034
!v10>                DAYVAL   = B0F(1,ISEASON,ISR,ISL)
!v10>                NITVAL   = B0F(2,ISEASON,ISR,ISL)
!v10> 
!v10> C Interpolation day/night with transitions at SAX (sunrise) and SUX (sunset)
!v10> 7034         SIPH(ISL) = HPOL(HOUR,DAYVAL,NITVAL,
!v10>      &                       SAX,SUX,1.,1.)
!v10>              end do
!v10>            end do
!v10>      
!v10> 
!v10> 
!v10> C Interpolation low/middle modip with transition at 30 degrees modip
!v10> 7033              SIPL(ISR) = SIPH(1) + (SIPH(2) - SIPH(1)) / DELA
!v10> 
!v10> C Interpolation low/high Rz12: linear from 10 to 100
!v10>        B0POL=SIPL(1)+(SIPL(2)-SIPL(1))/90.*(R-10.)
!v10>        RETURN
!v10>        END FUNCTION B0POL
!v10> c******
!v10> 
!v10> 
!v10> 
...
!v10> 
!v10> C
!v10>        REAL FUNCTION D1LAY ( X, XM, SC, HX )
!v10> C ------------------------------------------------------------ dLAY/dX
!v10>        implicit none
!v10>        real, intent(in) :: x,xm, sc, hx
!v10> 
!v10>         D1LAY = ( EPST(X,SC,HX) - EPST(XM,SC,HX) ) /  SC
!v10>        RETURN
!v10>        END FUNCTION D1LAY
!v10> C******
!v10> 
!v10> 
...
!v10> 
!v10> C
!v10>        REAL FUNCTION D2LAY ( X, XM, SC, HX )
!v10> C ---------------------------------------------------------- d2LAY/dX2
!v10>        implicit none
!v10>        real, intent(in) :: x,xm, sc, hx
!v10> 
!v10>        D2LAY = EPLA(X,SC,HX) /  (SC * SC)
!v10>        RETURN
!v10>        END FUNCTION D2LAY
!v10> C
!v10> 
!v10> 
!v10> 
...
C ------------------------------------------------------------ TRANSITION
       ...
       IF (ABS(D1) < ARGMAX)  then               ! GOTO 1
             eptr = 1.0d0 + exp(d1)
!1           EPTR = ALOG ( 1.0 + EXP( D1 ))      ! RETURN
             EPTR = ALOG ( eptr )                ! RETURN
       else
          IF (D1 >  0.0) THEN
             EPTR = D1
          ELSE
...
C ------------------------------------------------------------ PEAK 
       ...
       D1 = ( X - HX ) / SC
       IF (ABS(D1).LT.ARGMAX)  then ! GOTO 1
...
C----------------------------------------------------------------------
        ...
        real,intent(in)     :: h, hmf2, hx, sc, amp
...
!v10> 
!v10> C
!v10>        SUBROUTINE LNGLSN ( N, A1, B1, AUS)
!v10> C --------------------------------------------------------------------
!v10> C SOLVES QUADRATIC SYSTEM OF LINEAR EQUATIONS:
!v10> C
!v10> C       INPUT:       N       NUMBER OF EQUATIONS (= NUMBER OF UNKNOWNS)
!v10> C              A(N,N)       MATRIX (LEFT SIDE OF SYSTEM OF EQUATIONS)
!v10> C              B(N)       VECTOR (RIGHT SIDE OF SYSTEM)
!v10> C
!v10> C       OUTPUT:       AUS       =.TRUE.         NO SOLUTION FOUND
!v10> C                     =.FALSE.  SOLUTION IS IN  A(N,J) FOR J=1,N
!v10> C --------------------------------------------------------------------
!v10> C
!v10>        implicit none
!v10>        integer, intent(in) :: n
!v10>        real , intent(in out)   :: a1(5,5),b1(5)
!v10>        logical, intent(out) :: aus
!v10> 
!v10>        integer :: nn, k, L, izg, imax, m
!v10>        real    :: a,b , amax, hsp, azv
!v10>     
!v10>        DIMENSION       A(5,5), B(5), AZV(10)
!v10> !       LOGICAL              AUS
!v10>       
!v10>          a=a1
!v10>          b=b1
...
!v10>               IF (L > N) GOTO 111
!v10>               HSP = ABS( A(L,K) )
!v10>               IF (HSP <  1.0d-8) IZG = IZG + 1
!v10>               IF (HSP <= AMAX  ) GOTO 110
!v10> 111         IF (ABS(AMAX).GE.1.E-10) GOTO 133
!v10>               AUS = .TRUE.
!v10>               RETURN
!v10> 
!v10> 133         IF (IMAX.EQ.K) GOTO 112
!v10> 
!v10>               DO  L=K,N   ! 2
!v10>                      AZV(L+1)  = A(IMAX,L)
!v10>                      A(IMAX,L) = A(K,L)
!v10> 2                    A(K,L)    = AZV(L+1)
!v10>               end do
!v10> 
...
!v10>               B(K)    = AZV(1)
!v10> 
!v10> 112         IF (IZG.EQ.(N-K)) GOTO 1
!v10>               AMAX = 1. / A(K,K)
!v10>               AZV(1) = B(K) * AMAX
!v10> 
!v10>               DO  M=K+1,N  !3
!v10> 3                     AZV(M+1) = A(K,M) * AMAX
!v10>               end do
!v10> 
!v10>               DO 4 L=K+1,N
!v10>                      AMAX = A(L,K)
!v10>                      IF (ABS(AMAX).LT.1.E-8) GOTO 4
!v10>                      A(L,K) = 0.0
!v10>                      B(L) = B(L) - AZV(1) * AMAX
!v10>                    DO M=K+1,N  !5
!v10> 5                     A(L,M) = A(L,M) - AMAX * AZV(M+1)
!v10>                    end do
!v10> 
!v10> 4              CONTINUE
!v10> 
...
!v10> 
!v10>                   DO  L=K+1,N   !7
!v10> 7                     AMAX = AMAX + A(K,L) * A(N,L)
!v10>                   end do  
!v10>        
!v10>               ENDIF
!v10>               IF (ABS(A(K,K)).LT.1.E-6) THEN
!v10>                      A(N,K) = 0.0
!v10>               ELSE
!v10>                      A(N,K) = ( B(K) - AMAX ) / A(K,K)
!v10>               ENDIF
!v10> 6      CONTINUE
!v10> 
!v10>         a1=a
...
C
       implicit none
       integer, intent(in) :: n, m, m0, m1
       real, intent(in)    :: hm, sc, hx, w, x, y
       real, intent(out)   :: var

       LOGICAL              SING

       DIMENSION       VAR(N), HX(N), SC(N), W(M), X(M), Y(M)
       dimension       BLI(5), ALI(5,5), XLI(5,10)

       integer   :: m01, j, i, k
       real      :: scm, xli, bli,ali
       real      :: xxli, xxsci, xxhxi, xxk
C
       M01=M0+M1
       SCM=0
...
!3               XLI(I,K) = RLAY( X(K), HM, SC(I), HX(I) )
                 XLI(I,K) = RLAY( xXK, HM, xxSCI, xxHXI )
              end do

              DO  K=M0+1,M01  !4
                  xxk=x(k)
!4                XLI(I,K) = D1LAY( X(K), HM, SC(I), HX(I) )
                  XLI(I,K) = D1LAY( xXK,  HM, xxSCI, xxHXI )
              end do

              DO  K=M01+1,M  !5
                  xxk=x(k)
!5                XLI(I,K) = D2LAY( X(K), HM, SC(I), HX(I) )
!                 XLI(I,K) = D2LAY( xXK , HM, xxSCI, xxHXI )
                  XLI(I,K) = D2LAY( xXK ,     xxSCI, xxHXI )
              end do
...
2       CONTINUE
        ...
        DO 7  J=1,N   !7
         DO   K=1,M   ! 6
              ...
              BLI(J) = BLI(J) + W(K) * Y(K) * xXLI
           DO I=1,N  !6
!6            ALI(J,I) = ALI(J,I) + W(K) * XLI(I,K)  *xXLI
              ALI(J,I) = ALI(J,I) + W(K) * XLI(I,K)  *xXLI
           end do
         end do
7       CONTINUE
...
C
       implicit none
       integer, intent(in)     :: n
       real , intent(in out)   :: a1(5,5),b1(5)
       logical, intent(out)    :: aus

       integer :: nn, k, L, izg, imax, m
       real    :: a,b , amax, hsp, azv
...
!v10> 
!v10> C
!v10>        SUBROUTINE INILAY(NIGHT,XNMF2,XNMF1,XNME,VNE,HMF2,HMF1, 
!v10>      &                            HME,HV1,HV2,HHALF,HXL,SCL,AMP,IQUAL)
!v10> C-------------------------------------------------------------------
!v10> C CALCULATES AMPLITUDES FOR LAY FUNCTIONS
!v10> C D. BILITZA, DECEMBER 1988
!v10> C
!v10> C INPUT:       NIGHT       LOGICAL VARIABLE FOR DAY/NIGHT DISTINCTION
!v10> C              XNMF2       F2 PEAK ELECTRON DENSITY [M-3]
!v10> C              XNMF1       F1 PEAK ELECTRON DENSITY [M-3]
!v10> C              XNME       E  PEAK ELECTRON DENSITY [M-3]
!v10> C              VNE       ELECTRON DENSITY AT VALLEY BASE [M-3]
!v10> C              HMF2       F2 PEAK ALTITUDE [KM]
!v10> C              HMF1       F1 PEAK ALTITUDE [KM]
!v10> C              HME       E  PEAK ALTITUDE [KM]
!v10> C              HV1       ALTITUDE OF VALLEY TOP [KM]
!v10> C              HV2       ALTITUDE OF VALLEY BASE [KM]
!v10> C              HHALF       ALTITUDE OF HALF-F2-PEAK-DENSITY [KM]
!v10> C
!v10> C OUTPUT:       HXL(4)       HEIGHT PARAMETERS FOR LAY FUNCTIONS [KM] 
!v10> C              SCL(4)       SCALE PARAMETERS FOR LAY FUNCTIONS [KM]
!v10> C              AMP(4)       AMPLITUDES FOR LAY FUNCTIONS
!v10> C              IQUAL       =0 ok, =1 ok using second choice for HXL(1)
!v10> C                       =2 NO SOLUTION
!v10> C---------------------------------------------------------------  
!v10>         implicit none
!v10> 
!v10>        real, intent( in)    :: xnmf2, xnmf1, xnme, vne, hmf2
!v10>        real, intent(in)     :: hmf1, hme, hv1, hv2, hhaLf
!v10>        real, intent(out)    :: hxL
!v10>        integer, intent(out) :: iqual
!v10> 
!v10>        DIMENSION       XX(8),YY(8),WW(8),AMP(4),HXL(4),SCL(4)
!v10>        LOGICAL,intent(in)  ::    NIGHT
!v10>           Logical :: ssin  
!v10>           integer :: numcon, nc0
!v10>           real    :: xx,yy, ww, amp,  scL, aLogf, aLogef, xhalf
!v10>           real    :: scL0 , zet, hfff, xfff, hxL1t
!v10> c
!v10> c constants --------------------------------------------------------
!v10>           integer NUMLAY; parameter(    NUMLAY = 4)
!v10>           integer NC1   ; parameter(    NC1    = 2)
!v10>           real ALG102   ;
!v10>                ALG102 =ALOG10(2.) 
!v10> c
!v10> c constraints: xx == height       yy == log(Ne/NmF2)    ww == weights
!v10> c -----------------------------------------------------------------
!v10>               ALOGF = ALOG10(XNMF2)
!v10>               ALOGEF = ALOG10(XNME) - ALOGF
!v10>               XHALF=XNMF2/2.
!v10> 
!v10>               XX(1) = HHALF
!v10>               XX(2) = HV1
!v10>               XX(3) = HV2
!v10>               XX(4) = HME
!v10>               XX(5) = HME - ( HV2 - HME )
!v10>               YY(1) = -ALG102
!v10>               YY(2) = ALOGEF
!v10>               YY(3) = ALOG10(VNE) - ALOGF
!v10>               YY(4) = ALOGEF
!v10>               YY(5) = YY(3)
!v10>               YY(7) = 0.0
!v10>               WW(2) = 1.
...
!v10>               WW(4) = 5.
!v10> c
!v10> c geometric paramters for LAY -------------------------------------
!v10> c difference to earlier version:  HXL(3) = HV2 + SCL(3)
!v10> c
!v10> 
!v10>               SCL0 = 0.7 * ( 0.216 * ( HMF2 - HHALF ) + 56.8 )
!v10>               SCL(1)    = 0.8 * SCL0
!v10>                SCL(2)   = 10.
!v10>                 SCL(3)  = 9.
!v10>                  SCL(4) = 6.
...
!v10> c
!v10> C DAY CONDITION--------------------------------------------------
!v10> c earlier tested:        HXL(2) = HMF1 + SCL(2)
!v10> c 
!v10>            IF(NIGHT) GOTO 7711
!v10>               NUMCON = 8
!v10>               HXL(1) = 0.9 * HMF2
!v10>                 HXL1T  = HHALF
!v10>               HXL(2) = HMF1
...
!v10>                  WW(7) = 50.
!v10>                  WW(8) = 500.
!v10> c without F-region ----------------------------------------------
!v10>               IF(XNMF1.GT.0) GOTO 100
!v10> 
!v10>                      HXL(2)=(HMF2+HHALF)/2.
!v10>                      YY(6) = 0.
!v10>                      WW(6) = 0.
!v10>                      WW(1) = 1.
!v10>                      GOTO 7722
!v10> c with F-region --------------------------------------------
!v10> 100              YY(6) = ALOG10(XNMF1) - ALOGF
!v10>               WW(6) = 3.
!v10>               IF((XNMF1-XHALF)*(HMF1-HHALF).LT.0.0) THEN
!v10>                 WW(1)=0.5
!v10>               ELSE
!v10>                 ZET = YY(1) - YY(6)
!v10>                 WW(1) = EPST( ZET, 0.1, 0.15)
!v10>               ENDIF
!v10>               IF(HHALF.GT.HMF1) THEN
!v10>                 HFFF=HMF1
!v10>                 XFFF=XNMF1
!v10>               ELSE
!v10>                 HFFF=HHALF
...
!v10>                GOTO 7722
!v10> c
!v10> C NIGHT CONDITION---------------------------------------------------
!v10> c different HXL,SCL values were tested including: 
!v10> c       SCL(1) = HMF2 * 0.15 - 27.1       HXL(2) = 200.       
!v10> c       HXL(2) = HMF1 + SCL(2)              HXL(3) = 140.
!v10> c         SCL(3) = 5.                     HXL(4) = HME + SCL(4)
!v10> c       HXL(4) = 105.                     
!v10> c
!v10> 7711              NUMCON = 7
!v10>               HXL(1) = HHALF
!v10>               HXL1T  = 0.4 * HMF2 + 30.
!v10>               HXL(2) = ( HMF2 + HV1 ) / 2.
!v10>               HXL(4) = HME
!v10>               XX(6)  = HV2
!v10>               XX(7) = HME
!v10>               YY(6) = 0.0
...
!v10>               XFFF = XHALF
!v10> c
!v10> C are valley-top and bottomside point compatible ? -------------
!v10> C
!v10> 7722       IF((HV1-HFFF)*(XNME-XFFF).LT.0.0) WW(2)=0.5
!v10>        IF(HV1.LE.HV2+5.0) WW(2)=0.5
!v10> c
!v10> C DETERMINE AMPLITUDES-----------------------------------------
!v10> C
!v10>            NC0  = NUMCON-NC1
!v10>            IQUAL= 0
!v10> 2299       CALL LSKNM(NUMLAY,NUMCON,NC0,NC1,HMF2,SCL,HXL,WW,XX,YY,
!v10>      &              AMP,SSIN)
!v10> 
!v10>                  IF(IQUAL.gt.0) GOTO 1937
!v10>            IF((ABS(AMP(1)).GT.10.0).OR.(SSIN)) THEN
!v10>               IQUAL=1
!v10>               HXL(1)=HXL1T
!v10>               GOTO 2299
!v10>            ENDIF
!v10> 
!v10> 1937       IF(SSIN) IQUAL=2
!v10>            RETURN
!v10>  
!v10>            END SUBROUTINE INILAY
!v10> c
!v10> c*********
!v10> 
!v10> 
...
!v10> 
!v10>        subroutine ioncom(h,z,f,fs,t,cn)
!v10>         implicit none        
!v10> c---------------------------------------------------------------
!v10> c ion composition model
!v10> c A.D. Danilov and A.P. Yaichnikov, A New Model of the Ion
!v10> c   Composition at 75 to 1000 km for IRI, Adv. Space Res. 5, #7,
!v10> c   75-79, 107-108, 1985
!v10> c
!v10> c        h       altitude in km
!v10> c       z       solar zenith angle in radians
!v10> c       f       latitude in radians
!v10> c       fs       10.7cm solar radio flux
!v10> c       t       season (month)
!v10> c       cn(1)   O+  relative density in percent
!v10> c       cn(2)   H+  relative density in percent
!v10> c       cn(3)   N+  relative density in percent
!v10> c       cn(4)   He+ relative density in percent
!v10> c       cn(5)   NO+ relative density in percent
!v10> c       cn(6)   O2+ relative density in percent
!v10> c       cn(7)   cluster ions  relative density in percent
!v10> c---------------------------------------------------------------
!v10> cCCC
!v10>         integer, intent(in) :: t
!v10>         real, intent(in)    :: h,z,f,fs
!v10>         real, intent(out)   :: cn
!v10>        
!v10>         integer :: i,j
!v10>         real    :: po, p, ph, pn, phe, pno, po2, pcL, s,  hm
!v10>         real    :: cm, aLL, var, hx, betL, xpon, aLh, beth
!v10> 
!v10> 
!v10> 
!v10>        dimension       cn(7),cm(7),hm(7),alh(7),all(7),beth(7),
!v10>      &                     betl(7),p(5,6,7),var(6),po(5,6),ph(5,6),
!v10>      &                     pn(5,6),phe(5,6),pno(5,6),po2(5,6),pcl(5,6)
!v10> 
!v10>        data po/4*0.,98.5,4*0.,320.,4*0.,-2.59E-4,2.79E-4,-3.33E-3,
!v10>      &              -3.52E-3,-5.16E-3,-2.47E-2,4*0.,-2.5E-6,1.04E-3,
!v10>      &              -1.79E-4,-4.29E-5,1.01E-5,-1.27E-3/
!v10>        data ph/-4.97E-7,-1.21E-1,-1.31E-1,0.,98.1,355.,-191.,
!v10>      &              -127.,0.,2040.,4*0.,-4.79E-6,-2.E-4,5.67E-4,
!v10>      &              2.6E-4,0.,-5.08E-3,10*0./
!v10>        data pn/7.6E-1,-5.62,-4.99,0.,5.79,83.,-369.,-324.,0.,593.,
!v10>      &              4*0.,-6.3E-5,-6.74E-3,-7.93E-3,-4.65E-3,0.,-3.26E-3,
!v10>      &              4*0.,-1.17E-5,4.88E-3,-1.31E-3,-7.03E-4,0.,-2.38E-3/
!v10>        data phe/-8.95E-1,6.1,5.39,0.,8.01,4*0.,1200.,4*0.,-1.04E-5,
!v10>      &              1.9E-3,9.53E-4,1.06E-3,0.,-3.44E-3,10*0./ 
!v10>        data pno/-22.4,17.7,-13.4,-4.88,62.3,32.7,0.,19.8,2.07,115.,
!v10>      &              5*0.,3.94E-3,0.,2.48E-3,2.15E-4,6.67E-3,5*0.,
!v10>      &              -8.4E-3,0.,-3.64E-3,2.E-3,-2.59E-2/
!v10>        data po2/8.,-12.2,9.9,5.8,53.4,-25.2,0.,-28.5,-6.72,120.,
!v10>      &              5*0.,-1.4E-2,0.,-9.3E-3,3.3E-3,2.8E-2,5*0.,4.25E-3,
!v10>      &              0.,-6.04E-3,3.85E-3,-3.64E-2/
!v10>        data pcl/4*0.,100.,4*0.,75.,10*0.,4*0.,-9.04E-3,-7.28E-3,
!v10>      &              2*0.,3.46E-3,-2.11E-2/
!v10> 
!v10>        DO 8 I=1,5
!v10>         DO  J=1,6
!v10>               p(i,j,1)=po(i,j)
...
!v10>        do 5 i=1,7
!v10>          do 7 j=1,6
!v10>               var(j) = p(1,j,i)*cos(z) + p(2,j,i)*cos(f) +
!v10>      &                 p(3,j,i)*cos(0.013*(300.-fs)) +
!v10>      &                      p(4,j,i)*cos(0.52*(t-6.)) + p(5,j,i)
!v10> 7         continue
!v10>          cm(i)  = var(1)
!v10>          hm(i)  = var(2)
!v10>          all(i) = var(3)
...
!v10>           if(hx) 1,2,3
!v10> 
!v10> 1               cn(i) = cm(i) * exp( hx * (hx * all(i) + betl(i)) )
!v10>               goto 4
!v10> 
!v10> 2                cn(i) = cm(i)
!v10>               goto 4
!v10> 
!v10> C          Patch to avoid underflow:
!v10> C3               cn(i) = cm(i) * exp( hx * (hx * alh(i) + beth(i)) )
!v10>  3               cn(i) = 0.
!v10>                xpon =               hx * (hx * alh(i) + beth(i))
!v10>                if (xpon .gt. -69.) cn(i) = cm(i) * exp (xpon)
!v10> 
!v10> 4         continue
!v10>          if(cn(i).LT.0.005*cm(i)) cn(i)=0.
!v10>          if(cn(i).GT.cm(i)) cn(i)=cm(i)
!v10>          s=s+cn(i)
!v10> 5       continue
!v10> 
!v10>        do 6 i=1,7
!v10> 6              cn(i)=cn(i)/s*100.
!v10> 
!v10>        return
!v10>        end subroutine ioncom
!v10> C     
!v10> C     
!v10> C     
!v10> 
...
!v10> 
!v10> C     
!v10>       SUBROUTINE CIRA86(IDAY,SEC,GLAT,GLONG,STL,F107A,TINF,TLB,SIGMA)
!v10> C*******************************************************************
!v10> C     Calculates neutral temperature parameters for IRI using the
!v10> C     MSIS-86/CIRA 1986 Neutral Thermosphere Model. The subroutines
!v10> C     GTS5, GLOBE5 and GLOBL5 developed by A.E. Hedin (2/26/87) were
!v10> C     modified for use in IRI --------- D. Bilitza -------- March 1991
!v10> C     
!v10> C     INPUT:
!v10> C     IDAY - DAY OF YEAR 
!v10> C     SEC - UT(SEC)
!v10> C     GLAT - GEODETIC LATITUDE(DEG)
!v10> C     GLONG - GEODETIC LONGITUDE(DEG)
!v10> C     STL - LOCAL APPARENT SOLAR TIME(HRS)
!v10> C     F107A - 3 MONTH AVERAGE OF F10.7 FLUX
!v10> C     
!v10> C     OUTPUT: 
!v10> C     TINF - EXOSPHERIC TEMPERATURE (K)
!v10> C     TLB - TEMPERATURE AT LOWER BOUNDARY (K)
!v10> C     SIGMA - SHAPE PARAMETER FOR TEMPERATURE PROFILE
!v10> C     
!v10> C     **********************************************************************
!v10>       DIMENSION         PLG(9,4)
!v10>       DATA DR,DR2/1.72142E-2,0.0344284/,HR/.2618/,SR/7.2722E-5/,
!v10>      $     XL/1000./,TLL/1000./,DGTR/1.74533E-2/
!v10>       save plg, c, s, c2, c4, s2,
!v10>      $     stloc, ctloc, s2tloc, c2tloc, s3tloc, c3tloc
!v10> C     
!v10> C     CALCULATE LEGENDRE POLYNOMIALS
!v10> C     
!v10>       IF(XL.EQ.GLAT)   GO TO 15
!v10>       C = SIN(GLAT*DGTR)
!v10>       S = COS(GLAT*DGTR)
!v10>       C2 = C*C
!v10>       C4 = C2*C2
!v10>       S2 = S*S
!v10>       PLG(2,1) = C
!v10>       PLG(3,1) = 0.5*(3.*C2 -1.)
!v10>       PLG(4,1) = 0.5*(5.*C*C2-3.*C)
!v10>       PLG(5,1) = (35.*C4 - 30.*C2 + 3.)/8.
!v10>       PLG(6,1) = (63.*C2*C2*C - 70.*C2*C + 15.*C)/8.
!v10>       PLG(2,2) = S
!v10>       PLG(3,2) = 3.*C*S
!v10>       PLG(4,2) = 1.5*(5.*C2-1.)*S
!v10>       PLG(5,2) = 2.5*(7.*C2*C-3.*C)*S
!v10>       PLG(6,2) = 1.875*(21.*C4 - 14.*C2 +1.)*S
!v10>       PLG(7,2) = (11.*C*PLG(6,2)-6.*PLG(5,2))/5.
!v10>       PLG(3,3) = 3.*S2
!v10>       PLG(4,3) = 15.*S2*C
!v10>       PLG(5,3) = 7.5*(7.*C2 -1.)*S2
!v10>       PLG(6,3) = 3.*C*PLG(5,3)-2.*PLG(4,3)
!v10>       PLG(4,4) = 15.*S2*S
!v10>       PLG(5,4) = 105.*S2*S*C 
!v10>       PLG(6,4)=(9.*C*PLG(5,4)-7.*PLG(4,4))/2.
!v10>       PLG(7,4)=(11.*C*PLG(6,4)-8.*PLG(5,4))/3.
!v10>       XL=GLAT
!v10>  15   CONTINUE
!v10>       IF(TLL.EQ.STL)   GO TO 16
!v10>       STLOC = SIN(HR*STL)
!v10>       CTLOC = COS(HR*STL)
!v10>       S2TLOC = SIN(2.*HR*STL)
!v10>       C2TLOC = COS(2.*HR*STL)
!v10>       S3TLOC = SIN(3.*HR*STL)
!v10>       C3TLOC = COS(3.*HR*STL)
!v10>       TLL = STL
!v10>  16   CONTINUE
!v10> C     
!v10>       DFA=F107A-150.
!v10> C     
!v10> C     EXOSPHERIC TEMPERATURE
!v10> C     
!v10> C     F10.7 EFFECT
!v10>       T1 =  ( 3.11701E-3 - 0.64111E-5 * DFA ) * DFA
!v10>       F1 = 1. + 0.426385E-2 * DFA 
!v10>       F2 = 1. + 0.511819E-2 * DFA
!v10>       F3 = 1. + 0.292246E-2 * DFA
!v10> C     TIME INDEPENDENT
!v10>       T2 = 0.385528E-1 * PLG(3,1) + 0.303445E-2 * PLG(5,1)
!v10> C     SYMMETRICAL ANNUAL AND SEMIANNUAL
!v10>       CD14 = COS( DR  * (IDAY+8.45398) )
!v10>       CD18 = COS( DR2 * (IDAY-125.818) )
!v10>       CD32 = COS( DR  * (IDAY-30.0150) )
!v10>       CD39 = COS( DR2 * (IDAY-2.75905) )
!v10>       T3 = 0.805486E-2 * CD32 + 0.14237E-1 * CD18
!v10> C     ASYMMETRICAL ANNUAL AND SEMIANNUAL
!v10>       T5 = F1 * (-0.127371 * PLG(2,1) - 0.302449E-1 * PLG(4,1) ) * CD14
!v10>      &     - 0.192645E-1 * PLG(2,1) * CD39
!v10> C     DIURNAL
!v10>       T71 =  0.123512E-1 * PLG(3,2) * CD14
!v10>       T72 = -0.526277E-2 * PLG(3,2) * CD14
!v10>       T7 = ( -0.105531 *PLG(2,2) - 0.607134E-2 *PLG(4,2) + T71 ) *CTLOC
!v10>      4     + ( -0.115622 *PLG(2,2) + 0.202240E-2 *PLG(4,2) + T72 ) *STLOC
!v10> C     SEMIDIURNAL
!v10>       T81 = 0.386578E-2 * PLG(4,3) * CD14
!v10>       T82 = 0.389146E-2 * PLG(4,3) * CD14
!v10>       T8= (-0.516278E-3 *PLG(3,3) - 0.117388E-2 *PLG(5,3) +T81)*C2TLOC
!v10>      3     +( 0.990156E-2 *PLG(3,3) - 0.354589E-3 *PLG(5,3) +T82)*S2TLOC
!v10> C     TERDIURNAL
!v10>       Z1 =  PLG(5,4) * CD14
!v10>       Z2 =  PLG(7,4) * CD14
!v10>       T14=(0.147284E-2*PLG(4,4)-0.173933E-3*Z1+0.365016E-4*Z2)*S3TLOC
!v10>      2     +(0.341345E-3*PLG(4,4)-0.153218E-3*Z1+0.115102E-3*Z2)*C3TLOC 
!v10>       T7814 = F2 * ( T7 + T8 + T14 )
!v10> C     LONGITUDINAL
!v10>       T11= F3 * (( 0.562606E-2 * PLG(3,2) + 0.594053E-2 * PLG(5,2) + 
!v10>      $     0.109358E-2 * PLG(7,2) - 0.301801E-2 * PLG(2,2) - 
!v10>      $     0.423564E-2 * PLG(4,2) - 0.248289E-2 * PLG(6,2) + 
!v10>      $     (0.189689E-2 * PLG(2,2) + 0.415654E-2 * PLG(4,2)) * CD14
!v10>      $     ) * COS(DGTR*GLONG) +
!v10>      $     ( -0.11654E-1 * PLG(3,2) - 0.449173E-2 * PLG(5,2) - 
!v10>      $     0.353189E-3 * PLG(7,2) + 0.919286E-3 * PLG(2,2) + 
!v10>      $     0.216372E-2 * PLG(4,2) + 0.863968E-3 * PLG(6,2) +
!v10>      $     (0.118068E-1 * PLG(2,2) + 0.331190E-2 * PLG(4,2)) * CD14
!v10>      $     ) * SIN(DGTR*GLONG) )
!v10> C     UT AND MIXED UT,LONGITUDE
!v10>       T12 = ( 1. - 0.565411 * PLG(2,1) ) * COS( SR*(SEC-31137.0) ) *
!v10>      $     (-0.13341E-1*PLG(2,1)-0.243409E-1*PLG(4,1)-0.135688E-1*PLG(6,1))
!v10>      $     +    ( 0.845583E-3 * PLG(4,3) + 0.538706E-3  * PLG(6,3) ) *
!v10>      $     COS( SR * (SEC-247.956) + 2.*DGTR*GLONG )
!v10> C     Exospheric temperature TINF/K  [Eq. A7]
!v10>       TINF = 1041.3 * ( 1. + T1+T2+T3+T5+T7814+T11+T12 ) * 0.99604
!v10> C     
!v10> C     TEMPERATURE DERIVATIVE AT LOWER BOUNDARY
!v10> C     
!v10> C     F10.7 EFFECT
!v10>       T1 =  0.252317E-2 * DFA 
!v10> C     TIME INDEPENDENT
!v10>       T2 = -0.467542E-1 * PLG(3,1) + 0.12026 * PLG(5,1) 
!v10> C     ASYMMETRICAL ANNUAL
!v10>       CD14 = COS( DR  * (IDAY+8.45398) )
!v10>       T5 = -0.13324 * PLG(2,1)  * CD14
!v10> C     SEMIDIURNAL
!v10>       ZZ = PLG(4,3) * CD14
!v10>       T81 = -0.973404E-2 * ZZ
!v10>       T82 = -0.718482E-3 * ZZ
!v10>       T8 =(0.191357E-1 *PLG(3,3) + 0.787683E-2 *PLG(5,3) + T81) *C2TLOC
!v10>      3     + (0.125429E-2 *PLG(3,3) - 0.233698E-2 *PLG(5,3) + T82) *S2TLOC
!v10> C     dTn/dh at lower boundary  [Eq. A6]
!v10>       G0 = 0.166728E2 * ( 1. + T1+T2+T5+T8 ) * 0.951363
!v10> C     
!v10> C     NEUTRAL TEMPERATURE AT LOWER BOUNDARY 120KM
!v10> C     
!v10>       CD9  = COS( DR2 * (IDAY-89.3820) )
!v10>       CD11 = COS( DR  * (IDAY+8.45398) )
!v10>       T1 = 0.568478E-3 * DFA
!v10>       T4 = 0.107674E-1 * CD9
!v10>       T5 =-0.192414E-1 * PLG(2,1) * CD11
!v10>       T7 = -0.2002E-1 *PLG(2,2) *CTLOC - 0.195833E-2 *PLG(2,2) *STLOC
!v10>       T8 = (-0.938391E-2 * PLG(3,3) - 0.260147E-2 * PLG(5,3)
!v10>      $     + 0.511651E-4 * PLG(6,3) * CD11 ) * C2TLOC
!v10>      $     + ( 0.131480E-1 * PLG(3,3) - 0.808556E-3 * PLG(5,3)
!v10>      $     + 0.255717E-2 * PLG(6,3) * CD11 ) * S2TLOC
!v10> C     Tn at lower boundary 120km   [Eq. A8]
!v10>       TLB = 386.0 * ( 1. + T1+T4+T5+T7+T8 ) * 0.976619
!v10> C     Sigma      [Eq. A5]
!v10>       SIGMA = G0 / ( TINF - TLB )
!v10>       RETURN
!v10>       END SUBROUTINE CIRA86
!v10> C
!v10> 
...
C
       real FUNCTION TN(H,TINF,TLBD,S)
C--------------------------------------------------------------------
...
C--------------------------------------------------------------------
        ...
        real :: zg2
       ZG2 = ( H  - 120.d0 ) * 6476.77d0 / ( 6356.77d0 + H )
...
c     real FUNCTION DTNDH(H,TINF,TLBD,S)
      real FUNCTION DTNDH(H,     TLBD,S)
C---------------------------------------------------------------------
       implicit none
c      real, intent(in) :: h, tinf, tLbd, s
