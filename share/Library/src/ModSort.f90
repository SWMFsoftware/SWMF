module ModSort
  ...
  public :: qsort_integer_2D
  public :: qsort_real_2D
...
5      iSort_I(l)=iSort_I(j)
  ...
  subroutine qsort_integer_2D(n, d, arr, testFunc, indx)

    ! Quick sort algorithm: sorts indx array according to 'arr' using
    ! user defined function 'func'
    ! 'func(a,b)' is any function that returns -1 for a < b, 0 for a = b
    ! or 1 for a > b

    ! Based on the F77 code indexx from Numerical Recipes
    integer, external :: testFunc
    integer, intent(in)  :: n, d

    integer, intent(in)     :: arr(d,n)
    integer, intent(out) :: indx(n)
...
1   if(ir-l < M)then
       do j=l+1,ir
          ...
          a=arr(:,indxt)
          do i=j-1,1,-1
             if(testFunc(arr(:,indx(i)), a) .le. 0)goto 2
             indx(i+1)=indx(i)
          end do
          i=0
2         indx(i+1)=indxt
       ...
       indx(k)=indx(l+1)
       indx(l+1)=itemp
       if(testFunc(arr(:,indx(l+1)),arr(:,indx(ir))) == 1)then
          itemp=indx(l+1)
          indx(l+1)=indx(ir)
          indx(ir)=itemp
       endif
       if(testFunc(arr(:,indx(l)), arr(:,indx(ir))) == 1) then
          itemp=indx(l)
          indx(l)=indx(ir)
          indx(ir)=itemp
       endif
       if(testFunc(arr(:,indx(l+1)), arr(:,indx(l))) == 1)then
          itemp=indx(l+1)
          indx(l+1)=indx(l)
          indx(l)=itemp
       endif
...
3      continue
       i=i+1
       if(testFunc(arr(:,indx(i)), a) == -1)goto 3
4      continue
       j=j-1
       if(testFunc(arr(:,indx(j)), a) == 1)goto 4
       if(j < i)goto 5
       itemp=indx(i)
       indx(i)=indx(j)
       indx(j)=itemp
...
5      indx(l)=indx(j)
  ...
  end subroutine qsort_integer_2D

  !=====================================================================
  subroutine qsort_real_2D(n, d, arr, testFunc, indx)
