...
module ModLinearSolver
  ...
  public :: test_linear_solver,test_cg
...
contains
  ...
  !============================================================================
  subroutine cg(matvec,R_I,Sol_I,n,IsInit,Tol,TypeStop,&
       Iter,iCommIn,preconditioner)
    ...
    ! Subroutine for applying the preconditioner
    optional::preconditioner
    interface
       subroutine preconditioner(a,b,n)
         ! Calculate b = M^{-1}.a where the preconditioner matrix
    ...
    integer, intent(in) :: n         !  number of unknowns.
    real, intent(inout) :: R_I(n)   ! right hand side vector
    real, intent(inout) :: Sol_I(n)  ! initial guess / solution vector
    logical, intent(in) :: IsInit    ! true  if Sol contains initial guess
    real,    intent(in) :: Tol       ! required / achieved residual
    ...
    integer, intent(out) :: Iter    ! maximum/actual number of iterations
    ...
    real, dimension(:),   allocatable :: P_I,aDotP_I, MMinusOneDotR_I
    !-----------------------------------------------------------------------
    ...
    ! Allocate arrays that used to be automatic
    allocate(P_I(n),aDotP_I(n))

    if(present(preconditioner))allocate(MMinusOneDotR_I(n))
    
   
    !-------------- compute initial residual vector --------------

    if(IsInit)then
       call matvec(Sol_I,ADotP_I,n)
       R_I = R_I - ADotP_I
    else
       Sol_I = 0.0
    end if
    ...
    LOOP: do
       ...
       if(present(preconditioner))then
          call preconditioner(R_I,MMinusOneDotR_I,n)
          rDotR = dot_product_mpi(R_I,MMinusOneDotR_I,iComm)
       else
          rDotR = dot_product_mpi(R_I,R_I,iComm)
       end if

       if(Iter == 0)then
          rDotR0 = rDotR
          if(rDotR0 == 0.0)return
       end if
      
       if(TypeStop=='abs'.and. rDotR <= Tol          )return
       if(TypeStop=='rel'.and. rDotR <= Tol * rDotR0 )return    
       
       if(present(preconditioner))then
          P_I = P_I + MMinusOneDotR_I/rDotR
       else
          P_I = P_I +             R_I/rDotR
       end if
      
       call matvec(P_I,aDotP_I,n)
      
       pDotADotP = dot_product_mpi(P_I,aDotP_I,iComm)
       R_I = R_I - aDotP_I/pDotADotP
       Sol_I = Sol_I + P_I/pDotADotP
       Iter = Iter + 1
    end do LOOP

    ! Deallocate arrays that used to be automatic
    deallocate(P_I,aDotP_I)
    if(allocated(MMinusOneDotR_I))deallocate(MMinusOneDotR_I)
    
  end subroutine cg
  !============================================================================
  subroutine test_cg
    implicit none
    real :: Temperature_I(101)
    real :: Source_I(101),Energy_I(101)

    !bi-temperature version
    real :: Energy2T_I(202),Temperature2T_I(202),Source2T_I(202)
    integer :: iX,Iter,iCount
    !---------------------------

    Temperature_I = 0.0
    Temperature_I(51) = 1000.0

    HeatConductionCoef_I = 1.0
    SpecificHeat_I = 1.0

    iCOunt=0
    write(*,*) 'Solve linear parabolic equation'

    do iStep = 1,100

       Source_I = SpecificHeat_I * Temperature_I

       call cg(heat_conduction,Source_I,Temperature_I,&
            101,.true.,1e-4,'rel',Iter)

       iCount=iCount+Iter

    end do
    write(*,*)'Compare with the exact solution'

    do iX = 1,101
       write(*,*)iX,Temperature_I(iX),50.0/sqrt(3.141593)*&
            exp(-2.50e-3*(iX-51)**2)
    end do
    ...
    !************************ Non-Linear equation ***********
    write(*,*) 'Solve non-linear parabolic equation'
    write(*,*) 'E_t=(4e-4*T^3T_x)_x, where E=T+2e-3*T^2)'
    ...
    !
    ! Below is the choice for the equation of state:
    Energy_I = Temperature_I + 2.0e-3*Temperature_I**2

    do iStep = 1,100
       
       !Specific heat is given by the temperatur derivative 
       !of the eos:
       
       SpecificHeat_I = 1.0 + 4.0e-3 * Temperature_I

       do iX=2,101
          !The heat conduction is chosen
          HeatConductionCoef_I(iX) = &
               2.0e-5*(Temperature_I(iX-1)**3 + Temperature_I(iX)**3 )
       end do
       
       Source_I = SpecificHeat_I * Temperature_I
       if(iStep==1)then
          call cg(heat_conduction,Source_I,Temperature_I,&
               101,.true.,1e-4,'abs',Iter,preconditioner = jacobi)
       else
          call cg(heat_conduction,Source_I,Temperature_I,&
               101,.true.,1e-4,'abs',Iter,preconditioner = jacobi)
       end if
       
       !Update energy using conservative scheme
    
       Energy_I(1) = Energy_I(1) + &
               HeatConductionCoef_I(2)*(Temperature_I(2)-Temperature_I(1)) 
       do iX=2,100
          Energy_I(iX) = Energy_I(iX) + &
               HeatConductionCoef_I(iX  )*(Temperature_I(iX-1)-Temperature_I(iX))+ &
               HeatConductionCoef_I(iX+1)*(Temperature_I(iX+1)-Temperature_I(iX)) 
       end do
 
       Energy_I(101) = Energy_I(101) + &
               HeatConductionCoef_I(101 )*(Temperature_I(100)-Temperature_I(101))

       !Recover the temperature from the internak energy

       Temperature_I =( -0.5 +sqrt(0.25 + 2.0e-3*Energy_I))*500.0
       iCount=iCount+Iter
    end do
  
    do iX = 1,101
       write(*,*)iX,Temperature_I(iX)
    end do
    write(*,*)iCount

    !***********Bi-temperature model*****************************************
    !First temperatute is for plasma (high heat capacity, low heat conduction
    !Second temperature stands for radiation (low heat capacity, high heat conduction)

    write(*,*) 'Solve bi-temperature heat transfer system'
    Temperature2T_I = 0.0
    Temperature2T_I(101:102) = 1000.0
    iCOunt=0
    
    !Assign the initial energy

    do iX = 1,101
       !
       ! Below is the choice for the equation of state for plasma (high heat capacity)
       
       Energy2T_I(2*iX-1) = 0.9* Temperature2T_I(2*iX-1) + 2.0e-3*Temperature2T_I(2*iX-1)**2
       
       ! Radiation energy density:
       Energy2T_I(2*iX  ) = 1e-1 *  Temperature2T_I(2*iX)
    end do
    ...
    do iStep = 1,100
       ...
       !of the eos:
       do iX = 1,101
          SpecificHeat2T_I(2*iX-1) = 0.9 + 4.0e-3 * Temperature2T_I(2*iX-1)
          SpecificHeat2T_I(2*iX  ) = 0.1 
       end do
 
       do iX=2,101
          !The heat conduction is chosen: for radiation
          HeatConductionCoef2T_I(2*iX) = &
               2.0e-5*(Temperature2T_I(2*iX-2)**3 + Temperature2T_I(2*iX)**3 )
          !The heat conduction in plasma is much lower:
          HeatConductionCoef2T_I(2*iX-1) = 0.0!&
               !1.0e-5*(Temperature2T_I(2*iX-3)**2.5 + Temperature2T_I(2*iX-1)**2.5 )
       end do
      
       
       Source2T_I = SpecificHeat2T_I * Temperature2T_I
 
       Energy2T_I = Energy2T_I - Source2T_I
       if(iStep==1)then
          call cg(heat_conduction_2t,Source2T_I,Temperature2T_I,&
               202,.true.,1e-4,'abs',Iter,preconditioner = jacobi_2t)
       else
          call cg(heat_conduction_2t,Source2T_I,Temperature2T_I,&
               202,.true.,1e-4,'abs',Iter,preconditioner = jacobi_2t)
       end if
       ...
       !Update energy using conservative scheme
       Energy2T_I = Energy2T_I + SpecificHeat2T_I * Temperature2T_I
  
      
       do iX=1,101
          !For plasma
          Temperature2T_I(2*iX-1) = &
               ( -0.45 +sqrt(0.2025 + 2.0e-3*abs(Energy2T_I(2*iX-1))))*500.0
          !For radiation
          Temperature2T_I(2*iX)   = (1.0e1 * Energy2T_I(2*iX))
       end do
       iCount=iCount+Iter
    end do
    write(*,*) 'Solution for bi-temperature model'
    write(*,*) 'X Te TRad'
    do iX = 1,101
       write(*,*)iX,Temperature2T_I(2*iX-1),Temperature2T_I(2*iX)
    end do
    write(*,*)iCount
  end subroutine test_cg
  !======================
  subroutine heat_conduction(TNPlusOne_I,ADotTN_I,n)
    integer,intent(in)::n
    real,intent(in)::TNPlusOne_I(n)
    real,intent(out)::ADotTN_I(n)
    integer::iX
    ADotTN_I(1) = SpecificHeat_I(1)*TNPlusOne_I(1)+&
         HeatConductionCoef_I(2)*(TNPlusOne_I(1) - TNPlusOne_I(2)) 
         
    ADotTN_I(101) = SpecificHeat_I(101)*TNPlusOne_I(101)+& 
         HeatConductionCoef_I(101)*(TNPlusOne_I(101) - TNPlusOne_I(100))
    do iX=2,100
       ADotTN_I(iX) =SpecificHeat_I(iX)*TNPlusOne_I(iX) + &
            HeatConductionCoef_I(iX) *(TNPlusOne_I(iX) - TNPlusOne_I(iX-1))+&
            HeatConductionCoef_I(iX+1)*(TNPlusOne_I(iX)- TNPlusOne_I(iX+1))
    end do
  end subroutine heat_conduction
  !=============================
  subroutine jacobi(R_I,MMinusOneR_I,n)
    integer,intent(in)::n
    real,intent(in)::R_I(n)
    real,intent(out)::MMinusOneR_I(n)
    integer::iX
    MMinusOneR_I(1) = R_I(1)/( SpecificHeat_I(1) +&
         HeatConductionCoef_I(2)) 
         
    MMinusOneR_I(101) = R_I(101)/(SpecificHeat_I(101)+& 
         HeatConductionCoef_I(101))
    do iX=2,100
        MMinusOneR_I(iX) = R_I(iX)/(SpecificHeat_I(iX) + &
            HeatConductionCoef_I(iX) +HeatConductionCoef_I(iX+1))
    end do  
 
  end subroutine jacobi
  !=============================
  subroutine heat_conduction_2t(TNPlusOne_I,ADotTN_I,n)
    integer,intent(in)::n
    real,intent(in)::TNPlusOne_I(n)
    real,intent(out)::ADotTN_I(n)
    integer::iX
    !Heat conduction:
    ADotTN_I(1:2) = SpecificHeat2T_I(1:2)*TNPlusOne_I(1:2)+&
         HeatConductionCoef2T_I(3:4)*(TNPlusOne_I(1:2) - TNPlusOne_I(3:4))
    !Temperature relaxation:
    ADotTN_I(1:2) = ADotTN_I(1:2) + cRelaxation * TNPlusOne_I(1:2)
    ADotTN_I(1)   = ADotTN_I(1) - cRelaxation * TNPlusOne_I(2)
    ADotTN_I(2)   = ADotTN_I(2) - cRelaxation * TNPlusOne_I(1)
      
    !Heat conduction:   
    ADotTN_I(201:202) = SpecificHeat2T_I(201:202)*TNPlusOne_I(201:202)+& 
         HeatConductionCoef2T_I(201:202)*(TNPlusOne_I(201:202) - TNPlusOne_I(199:200))
    !Temperature relaxation:
    ADotTN_I(201:202) = ADotTN_I(201:202) + cRelaxation * TNPlusOne_I(201:202)
    ADotTN_I(201)   = ADotTN_I(201) - cRelaxation * TNPlusOne_I(202)
    ADotTN_I(202)   = ADotTN_I(202) - cRelaxation * TNPlusOne_I(201)

    do iX=2,100
       !Heat conduction:
       ADotTN_I(2*iX-1:2*iX) = SpecificHeat2T_I(2*iX-1:2*iX)*TNPlusOne_I(2*iX-1:2*iX) + &
            HeatConductionCoef2T_I(2*iX-1:2*iX  ) *(TNPlusOne_I(2*iX-1:2*iX) - TNPlusOne_I(2*iX-3:2*iX-2))+&
            HeatConductionCoef2T_I(2*iX+1:2*iX+2) *(TNPlusOne_I(2*iX-1:2*iX) - TNPlusOne_I(2*iX+1:2*iX+2))
       !Temperature relaxation:
       ADotTN_I(2*iX-1:2*iX) = ADotTN_I(2*iX-1:2*iX) + cRelaxation * TNPlusOne_I(2*iX-1:2*iX)
       ADotTN_I(2*iX-1     ) = ADotTN_I(2*iX-1     ) - cRelaxation * TNPlusOne_I(       2*iX)
       ADotTN_I(       2*iX) = ADotTN_I(       2*iX) - cRelaxation * TNPlusOne_I(2*iX-1     )
    end do
  end subroutine heat_conduction_2t
  !=============================
  subroutine jacobi_2t(R_I,MMinusOneR_I,n)
    integer,intent(in)::n
    real,intent(in)   ::R_I(n)
    real,intent(out)  ::MMinusOneR_I(n)
    integer :: iX
    real    :: Lambda_I(2) !Diagonal coefficients
    real    :: Det

    !-----------
    do iX=1,101
       if(iX==1)then
          Lambda_I = SpecificHeat2T_I(1:2    ) + HeatConductionCoef2T_I(3:4)     +cRelaxation
       elseif(iX==101)then
          Lambda_I = SpecificHeat2T_I(201:202) + HeatConductionCoef2T_I(201:202) +cRelaxation
       else
          Lambda_I = SpecificHeat2T_I(2*iX-1:2*iX  ) + HeatConductionCoef2T_I(2*iX-1:2*iX) + &
               HeatConductionCoef2T_I(2*iX+1:2*iX+2) + cRelaxation
       end if

       !Invert 2*2 matrix :
       ! || Lambda_1    cRelaxation ||
       ! || cRelaxation Lambda_2    ||
       ! Multiply the inverted matrix by R_I(2*iX-1:2*iX)

       Det = product(Lambda_I) - cRelaxation**2
       
       MMinusOneR_I(2*iX-1) = (R_I(2*iX-1)*Lambda_I(2) + R_I(2*iX  )* cRelaxation) / Det
       MMinusOneR_I(2*iX  ) = (R_I(2*iX  )*Lambda_I(1) + R_I(2*iX-1)* cRelaxation) / Det
         
    end do  
 
  end subroutine jacobi_2t
