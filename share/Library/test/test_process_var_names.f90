program process_var_names_test
  ...
  use ModProcessVarNames, ONLY: process_var_names
  use ModVarIndexes,      ONLY: NameVar_V
  use ModUtilities,       ONLY: lower_case
  implicit none

  integer  :: nDensity, nSpeed, nVar, iVar
  ...
  ! Fix string array to comply with usual input to process_var_names.
  ...
  NameVarFixed_V = NameVar_V
  do iVar = 1, nVar
     call lower_case(NameVarFixed_V(iVar))
  end do

  call process_var_names(nVar, NameVarFixed_V, nDensity, nSpeed)
  print*, 'Original  Standerdized  nDensity=',nDensity,'nSpeed=',nSpeed
  print*, '--------  ----------- '
  do iVar = 1, nVar
     print*,  NameVar_V(iVar),'         ',trim(NameVarFixed_V(iVar))
  end do
...
end program process_var_names_test
! =================================================================
