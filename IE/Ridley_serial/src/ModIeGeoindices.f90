...
contains
  ...
  subroutine get_index_mags(MagJhOut_DI, MagJpOut_DI)
    ...
    integer:: i, iError
    real, dimension(3,nIndexMag) :: XyzSmg_DI, MagJhLocal_DI, MagJpLocal_DI
    ...
    if( i /= nIndexMag + 1 ) call CON_stop(&
         NameSub//' Indexing error!  Not all magnetometers accounted for.')

    ! Collect Hall and Pedersen B perturbations.
    call iono_mag_perturb(nIndexMag, XyzSmg_DI, MagJhOut_DI, MagJpOut_DI)

    ! Add up perturbations from all the PEs
    if(nProc > 1)then 
       MagJhLocal_DI = MagJhOut_DI
       MagJpLocal_DI = MagJpOut_DI
       call MPI_allreduce(MagJhLocal_DI, MagJhOut_DI, 3*nIndexMag, &
            MPI_REAL, MPI_SUM, iComm, iError)
       call MPI_allreduce(MagJpLocal_DI, MagJpOut_DI, 3*nIndexMag, &
            MPI_REAL, MPI_SUM, iComm, iError)
    end if
